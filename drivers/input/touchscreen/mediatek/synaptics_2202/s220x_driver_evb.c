...
void synaptics_power ( unsigned int on )
{
	...
	if ( on )
	{
		...
		TPD_LOG ( "turned on the power ( VGP1, 2 )\n" );
		msleep(100);
	}
	else
	{
		...
		TPD_LOG ( "turned off the power ( VGP1, 2 )\n" );
		msleep(5);
	}
}
...
#else
		i2c_msgs.msg = kcalloc(message_count, sizeof(i2c_msgs.msg), GFP_KERNEL);	
#endif		
...
static int synaptics_touch_event_handler ( void *unused )
{
	...
	do
	{
		...
		{
			/* read button data */
			TPD_LOG ("button_page=%x, button_data=%x\n",button_page,button_data);
...
static int synaptics_firmware_check ( struct i2c_client *client )
{
	...
	else
	{
		...
		if ( ( !strcmp ( fw_product_id, image_product_id ) ) && ( image_ver > fw_ver ) || 1 ) //for update
		{
...
void synaptics_init_sysfs ( struct i2c_client *client )
{
	TPD_FUN ();
	int err;
...
static int synaptics_i2c_probe ( struct i2c_client *client, const struct i2c_device_id *id )
{
	...
	memset ( &i2c_msgs, 0, sizeof(struct st_i2c_msgs));
	tpd_i2c_client = client;
#if defined(LGE_USE_SYNAPTICS_F54)
	ds4_i2c_client = client;
#endif

	synaptics_init_sysfs ( client );
