...
#define	TPD_DEV_NAME						"synaptics_220x"
...
#define LPWG_STATUS_REG						(finger.data_base+28)
...
static int g_mode_set_status=0;
...
static void synaptics_power_control(unsigned int on);
#ifdef WORKQUEUE_FW
...
int synaptics_power_vbus( unsigned int on )
{
	...
	if ( on )
	{
		...
		TPD_LOG ( "DEVICE_CONTROL_REG : 0x%2x\n", r_mem );
	...
	else
	{
		...
		r_mem =( r_mem & 0xF8) | SENSOR_SLEEP_MASK;
	...
	r_mem = 0x01;
	if ( synaptics_ts_write ( tpd_i2c_client, TWO_D_COMMAND, &r_mem, 1 ) < 0 )
	{
...
3. vddl off
*/
static void synatpics_codinate_power_contorl()
{	
	TPD_FUN();
	TPD_ERR( "Power_status_cnt : 0x%x\n", Power_status_cnt);
	
	if ( Power_status_cnt == 0 || Power_status_cnt == 1 ) return ;
	
	if ( Power_status_cnt < 0 ) 
	{
		hwPowerOn ( MT6323_POWER_LDO_VGP2, VOL_3000, "TP" );
		Power_status_cnt++;
		synatpics_codinate_power_contorl();
	}
	else if ( Power_status_cnt >=2 ) 
	{
...
static void synaptics_power_control(unsigned int on)
{
	...
	TPD_LOG("Power_status : 0x%x /Power_status_cnt : 0x%x \n", Power_status, Power_status_cnt);
	TPD_FUN();
}
void synaptics_power ( unsigned int on )
{
	...
	{		
		...
		synaptics_reset_pin ( GPIO_TOUCH_RESET_N ); // first
#endif
		msleep(delay_time.booting);
		/* replace VGP1, 2 setting */
		if ( synaptics_power_vbus ( on ) )
			TPD_LOG ( "turned 0x%x the power ( VGP1,2 by reg )\n", on );	
...
int synaptics_ic_ctrl ( struct i2c_client *client, u8 code, u16 value )
{
	...
	TPD_ERR ( "synaptics_ic_ctrl: %d, %d\n", code, value );

	if ( Power_status == IC_POWER_OFF )
		synaptics_power_control(1);
		...
		case IC_CTRL_DOUBLE_TAP_WAKEUP_MODE:
			switch ( value )
			{
				case 0:  /* touch double-tap disable */
					...
					TPD_LOG ( "TWO_D_REPORT_MODE_REG : 0x%2x\n", r_mem );
					break;
				...
				case 1:  /* touch double-tap enable */
					...
					{
						//r_mem = ( r_mem & 0xF8 ) | WAKEUP_GESTURE_REPORTING_MODE_MASK; //why 0?
						r_mem = 0x8c;
						if ( synaptics_ts_write ( client, TWO_D_REPORT_MODE_REG, &r_mem, 1 ) < 0 )
						{
							TPD_ERR ( "TWO_D_REPORT_MODE_REG write fail\n" );
...
static void synaptics_reset_pin_power ( int on )
{
	...
	TPD_ERR ("on_off:0x%x\n", on);
	mt_set_gpio_mode ( GPIO_TOUCH_RESET_N, GPIO_MODE_00 );
...
static void synaptics_ic_reset ( void )
{
	...
	if ( mt_get_gpio_dir ( GPIO_CTP_EINT_PIN ) == GPIO_DIR_IN )
...
static int synaptics_ic_status_check ( u8* int_status )
{
	int ret;
	...
	if ( ret < 0 )
	{
		TPD_ERR ( "INTERRUPT_STATUS_REG read fail by reset pin\n" );
		return -1;
	}
	if ( (*int_status) != 0x4 )
		TPD_LOG("intt_status : 0x%x\n", *int_status);
...
static int synaptics_knock_lpwg ( struct i2c_client* client, u32 code, u32 value, struct point *data )
{
	...
	u8 buf = 0;
	int i = 0, ret = 0;
	if ( mt_get_gpio_out ( GPIO_TOUCH_RESET_N) == GPIO_OUT_ZERO )
		synaptics_reset_pin_power(1);
...
static void synaptics_knock_set_work ( struct delayed_work *knock_set_work )
{	
	TPD_ERR("ENTER - 0x%x, 0x%x\n", g_mode_set_status, suspend_status);
	TPD_LOG ( "lpwg_mode, double_tap_enable, multi_tap_enable : %d, %d, %d\n", lpwg_mode, double_tap_enable, multi_tap_enable );
	...
	switch ( g_mode_set_status )
		{
			case LPWG_NONE:
				synaptics_power_vbus(0);	
			if ( Power_status == IC_POWER_ON )
				synaptics_power_control(0);
			break;
...
static void synaptics_knock_suspend ( void )
{
	...
	g_mode_set_status = knock_on_enable = 1;
	if ( !download_status && suspend_status )
...
static void synaptics_knock_resume ( void )
{
	...
	g_mode_set_status = knock_on_enable = 0;
	if ( suspend_status )
		queue_delayed_work ( knock_set_work_wq, &knock_set_work, 0 );
}
...
static int synaptics_touch_event_handler ( void *unused )
{
	...
	do
	{
		...
		ret = synaptics_ic_status_check (&int_status);
		if ( ret != 0 )
		{
			goto exit_work;
		}
...
//		TPD_LOG ("int_status = 0x%x\n", int_status);
		if ( int_status == 0x40 || int_status == 0 || int_status == 0x2 )
			goto exit_work;
...
static enum hrtimer_restart synaptics_notify_timer_handler ( struct hrtimer *notify_timer )
{
	...
	if ( suspend_status )
	{
		g_mode_set_status = lpwg_mode;
		queue_delayed_work ( knock_set_work_wq, &knock_set_work, 0);
	}
	else
	{		
		g_mode_set_status = LPWG_ACTIVE_MODE;
		queue_delayed_work ( knock_set_work_wq, &knock_set_work, 0 );
	}
	
	TPD_ERR("knock_on_enable : 0x%x, suspend_status : 0x%x\n", knock_on_enable, suspend_status);
...
static ssize_t store_lpwg_notify ( struct device *dev, struct device_attribute *attr, const char *buf, size_t size )
{
	...
	TPD_LOG ( "touch notify type = %d , value[0] = %d, value[1] = %d, value[2] = %d, value[3] = %d ", type, value[0], value[1], value[2], value[3] );

#ifdef LGE_USE_PROXI_SENSOR
	if ( type == 6 ) return size;
#endif
	...
	case 6:
        ...
        case 7:	
			...
			if ( hrtimer_try_to_cancel ( &notify_timer ) < 0 )
			{
				...
				TPD_ERR("suspend_status : 0x%x\n", suspend_status);		

//			wake_lock_timeout( &notify_lock, msecs_to_jiffies(500));
			hrtimer_start ( &notify_timer, ktime_set(0, MS_TO_NS(200)), HRTIMER_MODE_REL );
			TPD_LOG("EXIT- timer run before enter sleep \n");
            break;
#endif //                    
		default:
			break;
...
static ssize_t synaptics_show_gpio_interr ( struct device *dev, struct device_attribute *attr, char *buf, size_t size )
{
...
static DEVICE_ATTR ( get_gpio_interr, 0664, synaptics_show_gpio_interr, NULL );
...
//	temp = temp | CONFIGURED_MASK;
	...
	multi_tap_enable = 0;
#else
	//both the suspend and the resume
	synaptics_knock_tap_control ( suspend_status );
#endif
...
void synaptics_init_sysfs ( void )
{
	...
	/*for debugging */
	err = device_create_file ( touch_fw_dev, &dev_attr_get_gpio_interr );
	if ( err )
	{
		TPD_ERR ( "Touchscreen : [get] touch device_create_file Fail\n" );
		device_remove_file ( touch_debug_dev, &dev_attr_get_gpio_interr );
	}

	err = device_create_file ( touch_fw_dev, &dev_attr_debug_ic_reset );
	if ( err )
	{
...
static int synaptics_i2c_probe ( struct i2c_client *client, const struct i2c_device_id *id )
{
	...
	/* Check for TOUCH IC*/
	ret = synaptics_ts_read ( tpd_i2c_client, F34_FUNCTION_EXIST, 1, &function_exist );
	if ( ret < 0 )
	{
		TPD_ERR ( "F34_FUNCTION_EXIST read fail Device NOT Found ... \n" );
/*                                                                                     */
		mt_set_gpio_mode(GPIO_TOUCH_I2C_SCL, GPIO_TOUCH_I2C_SCL_M_GPIO );
		mt_set_gpio_mode(GPIO_TOUCH_I2C_SDA, GPIO_TOUCH_I2C_SDA_M_GPIO );
/*                                                                                     */
		ret = -ENODEV;
		goto err_check_functionality_failed;
	}
	...
	{
		if ( function_exist == 0x34) 
			TPD_ERR("  Device Found ..\n");
		else
			TPD_ERR("  Device Found[%d]  ..\n", function_exist);
	}
	...
	if ( knock_set_work_wq )	
		INIT_DELAYED_WORK ( &knock_set_work, synaptics_knock_set_work );	
...
static void synaptics_suspend ( struct early_suspend *h )
{
	...
	if ( !knock_on_enable && !download_status )
	{
		...
		if ( synaptics_power_vbus (0 ) )
			TPD_LOG ( "turned 0x%x the power ( VGP1,2 by reg ) in suspend \n", 0 );
