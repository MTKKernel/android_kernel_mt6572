...
static u32 lcd_touch_ratio_x;
static u32 lcd_touch_ratio_y;
...
#if defined(LGE_USE_SYNAPTICS_FW_UPGRADE)
char fw_path[256];
static unsigned char *fw_start;
unsigned long fw_size;
bool fw_force_update;
static bool download_status;		/* avoid power off during F/W upgrade */
bool suspend_status;		/* avoid power off during F/W upgrade */
bool key_lock_status;
static struct wake_lock fw_suspend_lock;
static struct device *touch_fw_dev;
...
static void synaptics_reset_pin_power ( int on )
{
	...
	if ( on )
	{
		mt_set_gpio_mode ( GPIO_TOUCH_RESET_PIN, GPIO_MODE_00 );
		mt_set_gpio_dir ( GPIO_TOUCH_RESET_PIN, GPIO_DIR_OUT );
		mt_set_gpio_out ( GPIO_TOUCH_RESET_PIN, GPIO_OUT_ONE );
	...
	else
	{
		mt_set_gpio_mode ( GPIO_TOUCH_RESET_PIN, GPIO_MODE_00 );
		mt_set_gpio_dir ( GPIO_TOUCH_RESET_PIN, GPIO_DIR_OUT );
		mt_set_gpio_out ( GPIO_TOUCH_RESET_PIN, GPIO_OUT_ZERO );
	}
}
...
static int synaptics_firmware_check ( struct i2c_client *client )
{
	...
	else
	{
		...
		if ( ( !strcmp ( fw_product_id, image_product_id ) ) && ( fw_ver < image_ver ) )
		{
...
static int synaptics_i2c_probe ( struct i2c_client *client, const struct i2c_device_id *id )
{
	...
	if ( synaptics_ts_read ( client, 0x007F, 10, &fw_product_id[0] ) < 0 )
	{
		TPD_ERR ( "Touch IC is not Synaptics\n" );
		return -1;
	}
	else if ( ( fw_product_id[3] != 0x33 ) || ( fw_product_id[4] != 0x34 ) || ( fw_product_id[5] != 0x36 ) )
	{
		TPD_LOG ( "Product ID is fail (%s)\n", &fw_product_id[3] );
		//return -1;
	}
	SEL_SYSFS = 1;
