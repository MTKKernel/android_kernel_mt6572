...
static void pchr_turn_on_charging (void)
{
    ...
    else
    {
		...
		#if defined(CONFIG_LGE_MINIABB)
			//do charging current set & enable & eoc voltage
...
#if defined(CONFIG_LGE_MINIABB)
	if(BMT_status.total_charging_time==10)
#endif		
...
#if defined(CONFIG_LGE_MINIABB)
	if(check_EOC_status()==KAL_TRUE ||(BMT_status.TOPOFF_charging_time >= MAX_CV_CHARGING_TIME))
#else
