...
#if defined(TARGET_S2) || defined(TARGET_S5)  /*                                                                                */
extern int dump_lcm_register(char *buf);
static ssize_t show_dump_lcm(struct device *device, struct device_attribute *attr, char *buf)
{
        u8 ret;
        char result[128] = {0x00};
        ret = dump_lcm_register(result);

        if(ret == true)  // register data changed
        {
            return sprintf(buf, "Data Changed Register : %s\n", result);
        }
        else  // register data NOT changed
        {
            return sprintf(buf, "Data NOT changed\n");
        }
}
#endif  /*                                                                                */
...
#endif
#if defined(TARGET_S2) || defined(TARGET_S5)  /*                                                                                */
        __ATTR(dump_lcm, S_IRUGO|S_IWUSR ,show_dump_lcm, NULL),
#endif  /*                                                                                */
};
