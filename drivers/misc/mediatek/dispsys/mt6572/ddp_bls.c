...
#else
#define CONFIG_LGE_MTK_LCDC_LUT
#endif

#if defined(CONFIG_LGE_MTK_LCDC_LUT)
#include "lge_mlut.h"
extern bool LGE_FacReadLCDCalibration(unsigned char *lcd_kcal);
extern bool LGE_FacWriteLCDCalibration(unsigned char *lcd_kcal);
extern int g_kcal_r;
extern int g_kcal_g;
extern int g_kcal_b;
#endif
...
void disp_bls_update_gamma_lut(void)
{
...
#if defined(CONFIG_LGE_MTK_LCDC_LUT)  /*                                                             */
        DISP_REG_SET(DISPSYS_BLS_LUT_UPDATE, 0x01);
        {
            int t = 0;
            u16 r, g, b;
            for(t = 0; t < 256; t++)
            {
                r = ((p_lg_mtk_lcdc_lut[t] & R_MASK) >> R_SHIFT);
                g = ((p_lg_mtk_lcdc_lut[t] & G_MASK) >> G_SHIFT);
                b = ((p_lg_mtk_lcdc_lut[t] & B_MASK) >> B_SHIFT);

                r = (scaled_by_kcal(r, g_kcal_r) << 2);
                g = (scaled_by_kcal(g, g_kcal_g) << 2);
                b = (scaled_by_kcal(b, g_kcal_b) << 2);
                // DISP_REG_SET((DISPSYS_BLS_GAMMA_LUT + (t*4)), (r<<20)|(g<<10)|(b<<0));
                DISP_REG_SET(DISP_REG_BLS_GAMMA_LUT(t), (r<<20)|(g<<10)|(b<<0));
            }
        }
        ...
        CurVal = (((((p_lg_mtk_lcdc_lut[255] & R_MASK) >> R_SHIFT) << 2)<<20) | ((((p_lg_mtk_lcdc_lut[255] & G_MASK) >> G_SHIFT) << 2)<<10) | (((p_lg_mtk_lcdc_lut[255] & B_MASK) >> B_SHIFT)<<2));
...
#if 1  /*                                                                               */
    DISP_REG_SET(DISP_REG_BLS_DITHER(6)         ,0x00000005);
    DISP_REG_SET(DISP_REG_BLS_DITHER(7)         ,0x00000222);
#else
...
/* output RGB888 */
    ...
    {
#if 1  /*                                                                               */
        DISP_REG_SET(DISP_REG_BLS_DITHER(15), 0x00000000);
        DISP_REG_SET(DISP_REG_BLS_DITHER(16), 0x00000000);
#else
...
#if defined(CONFIG_LGE_MTK_LCDC_LUT)  /*                                                             */
int get_kcal_value(unsigned char *rsp)
{
        int ret_val = 0;

        unsigned char lcd_kcal[20];
        unsigned char r = 0;
        unsigned char g = 0;
        unsigned char b = 0;
        unsigned char is_kcal = 0;

        memset(lcd_kcal, 0, sizeof(lcd_kcal));

        ret_val = LGE_FacReadLCDCalibration(lcd_kcal);
        ...
        if(is_kcal == 75/*'K'*/) // calibration OK .ascii code 'K' == 75
            sprintf(rsp, "%c%c%c%c", r, g, b, is_kcal);
        else
            sprintf(rsp, "%c%c%c%c", g_kcal_r, g_kcal_r, g_kcal_r, is_kcal);

        return ret_val;
}
EXPORT_SYMBOL(get_kcal_value);

int set_kcal_value(unsigned char r, unsigned char g, unsigned char b)
{
        int ret_val = 0;
        unsigned char buf[20];

        memset(buf, 0, sizeof(buf));
        sprintf(buf, "%c%c%cK", r, g, b);

        LGE_FacWriteLCDCalibration(buf);
        return ret_val;
}
EXPORT_SYMBOL(set_kcal_value);

int update_kcal_register(unsigned char r, unsigned char g, unsigned char b)
{
        unsigned long CurVal;

        // TODO : AT cmd check this point
        //if(DISP_REG_GET(DISP_REG_BLS_EN) & 0x1)
//        {
//            DISP_ERR("try to update gamma lut while BLS is active\n");
//            return;
//        }
        ...
        DISP_REG_SET(DISP_REG_BLS_LUT_UPDATE, 0x1);
        //  LCD_OUTREG32(&LCD_REG->GAMCON, 0x30306);
        //  LCD_OUTREG32(&LCD_REG->WROI_DATA_ADDR, 0x10);
        {
            int t = 0;
            u16 r, g, b;
            for(t = 0; t <256; t++)
            {
                ...
                g = (scaled_by_kcal(g, g_kcal_g)<<2);
                b = (scaled_by_kcal(b, g_kcal_b)<<2);
                // LCD_OUTREG32(&(LCD_REG->GAMMA[t]), (r<<16)|(g<<8)|(b<<0));  //R/G/B Channel Gamma Value for input=r,g,b
                DISP_REG_SET(DISP_REG_BLS_GAMMA_LUT(t),(r<<20)|(g<<10)|(b<<0) );
            }
        }
        DISP_REG_SET(DISP_REG_BLS_GAMMA_SETTING, 0x00000001);
        ...
        CurVal = (((((p_lg_mtk_lcdc_lut[255] & R_MASK) >> R_SHIFT) << 2)<<20) | ((((p_lg_mtk_lcdc_lut[255] & G_MASK) >> G_SHIFT) << 2)<<10) | (((p_lg_mtk_lcdc_lut[255] & B_MASK) >> B_SHIFT)<<2));
        DISP_REG_SET(DISP_REG_BLS_GAMMA_BOUNDARY, CurVal);
        DISP_REG_SET(DISPSYS_BLS_LUT_UPDATE,0x00);
}
EXPORT_SYMBOL(update_kcal_register);
#endif  /*                                                             */
