...
void BM_Enable(const unsigned int enable)
{
    ...
    emi_reg_sync_writel((value & ~(BUS_MON_PAUSE | BUS_MON_EN)) | (enable ? BUS_MON_EN : 0), EMI_BMEN);
}
...
{
    return (counter_num < 1 || counter_num > BM_COUNTER_MAX) ? BM_ERR_WRONG_REQ : emi_readl(EMI_TTYPE1 + (counter_num - 1) * 8);
}
...
int BM_SetMonitorCounter(const unsigned int counter_num, const unsigned int master, const unsigned int trans_type)
{
    ...
    if (counter_num == 1) {
        addr = EMI_BMEN;
        value = (emi_readl(addr) & ~(iMask << 16)) | ((trans_type & 0xFF) << 24) | ((master & 0x7F) << 16);
    }else {
...
{
  unsigned int value;
  value = emi_readl(EMI_BMEN2) & ~(0b11 << 24);
