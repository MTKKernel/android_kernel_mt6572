...
void LM_Enable(const unsigned int enable)
{
...
unsigned int LM_IsOverrun(void)
{
    /*
    * return !0 if EMI_BCNT(bus cycle counts) or EMI_WACT(total word counts) is overrun,
    * otherwise non-overrun return an 0 value
...
int LM_SetMaster(const unsigned int master)
{
    unsigned int value, addr;
...
int LM_GetWTransCount(void)
{
    return readl(EMI_WTCT);
}

int LM_GetWLatCount(void)
{
    return readl(EMI_WLCT);
}

int LM_GetRTransCount(void)
{
    return readl(EMI_RTCT);
}

int LM_GetRLatCount(void)
{
