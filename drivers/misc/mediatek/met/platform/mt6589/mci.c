...
static void mci_value_init(void)
{
	...
	MCI_Event_Set(0x0, 0x1); /*0x2: AC_R snoop transaction counts; 0x4: AC_R snoop hit*/
}
...
static unsigned int mci_polling(unsigned int *value)
{
	...
	value[++j] = MCI_GetEventCount(0);
	value[++j] = MCI_GetEventCount(1);
	//MCI_Counter_Reset();
	//MCI_Event_Read();
	MCI_Mon_Disable();
	MCI_Mon_Enable();
...
static int mci_print_header(char *buf, int len)
{
	...
	return snprintf(buf, PAGE_SIZE, header,
		evt0, mci_desc[evt0].name, evt1, mci_desc[evt1].name);
}
