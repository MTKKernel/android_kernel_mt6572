...
#if 0 
	p_bmm050->dev_addr = BMM050_I2C_ADDR;
#else
	if(system_rev<1) // EVB 
	{
		p_bmm050->dev_addr = BMM050_I2C_ADDR_EVB;		
	}
	else if(system_rev==1)
	{
		p_bmm050->dev_addr = BMM050_I2C_ADDR_REV_A;				
	}
	else
		p_bmm050->dev_addr = BMM050_I2C_ADDR_REV_A;		
#endif 
...
#define MSE_ERR(fmt, args...)		printk(KERN_ERR MSE_TAG"[ERROR] %s() line=%d : "fmt, __FUNCTION__, __LINE__, ##args)
...
#ifdef BMC050_GEST_FLIP
static int flip_data[CALIBRATION_DATA_SIZE];
#endif
...
#ifdef BMC050_GEST_FLIP
static int flip_delay = BMM050_DEFAULT_DELAY;
#endif/*BMC050_GEST_FLIP*/
...
#endif //BMC050_VRV
#ifdef BMC050_GEST_FLIP
static atomic_t flip_flag = ATOMIC_INIT(0);
#endif/*BMC050_GEST_FLIP*/
...
#else
static struct i2c_board_info __initdata bmm050_i2c_info = {I2C_BOARD_INFO(BMM050_DEV_NAME, 0x00)};
#endif
...
#endif //BMC050_VRV
#ifdef BMC050_GEST_FLIP
	case 13:	/* SENSOR_HANDLE_GEST_FLIP */
		memcpy(flip_data, buf+1, 4*sizeof(int));
		break;
#endif/*BMC050_GEST_FLIP*/
	default:
		break;
...
#if defined(BMC050_GEST_FLIP)
		MSE_LOG("Get flip data: %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d!\n",
			flip_data[0],flip_data[1],flip_data[2],flip_data[3],
			flip_data[4],flip_data[5],flip_data[6],flip_data[7],
			flip_data[8],flip_data[9],flip_data[10],flip_data[11]);
#endif/*BMC050_GEST_FLIP*/
...
static long bmm050_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	switch (cmd)
	{
...
#ifdef BMC050_GEST_FLIP
		case FLIP_IOCTL_GET_DELAY: 		//used by daemon
			if(copy_to_user(argp, &flip_delay, sizeof(flip_delay)))
			{
				MSE_ERR("copy_to_user failed.");
				return -EFAULT;
			}
			/* clear the flag */
			mutex_lock(&uplink_event_flag_mutex);
			if ((uplink_event_flag & BMMDRV_ULEVT_FLAG_FLIP_DELAY) != 0)
			{
				uplink_event_flag &= ~BMMDRV_ULEVT_FLAG_FLIP_DELAY;
			}
			mutex_unlock(&uplink_event_flag_mutex);
			/* wake up the wait queue */
			wake_up(&uplink_event_flag_wq);
		...
		case FLIP_IOCTL_GET_FLAG:		//used by daemon
			sensor_status = atomic_read(&flip_flag);
#ifdef BMC050_BLOCK_DAEMON_ON_SUSPEND
			if ((sensor_status == 1) && (atomic_read(&driver_suspend_flag) == 1))
			{
				/* de-active vrv-channel when driver suspend regardless of vrv_flag*/
			...
			/* clear the flag */
			mutex_lock(&uplink_event_flag_mutex);
			if ((uplink_event_flag & BMMDRV_ULEVT_FLAG_FLIP_ACTIVE) != 0)
			{
				uplink_event_flag &= ~BMMDRV_ULEVT_FLAG_FLIP_ACTIVE;
			}
			mutex_unlock(&uplink_event_flag_mutex);
			/* wake up the wait queue */
			wake_up(&uplink_event_flag_wq);
...
/*----------------------------------------------------------------------------*/
#ifdef BMC050_GEST_FLIP
int bmm050_flip_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value;
	hwm_sensor_data* gest_flip_data;
#if DEBUG
	struct i2c_client *client = this_client;
	struct bmm050_i2c_data *data = i2c_get_clientdata(client);
#endif
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			{
				gest_flip_data = (hwm_sensor_data *)buff_out;
				mutex_lock(&sensor_data_mutex);

				gest_flip_data->values[0] = flip_data[0];
				gest_flip_data->values[1] = flip_data[1];
				gest_flip_data->values[2] = flip_data[2];
				gest_flip_data->status = flip_data[3];
				gest_flip_data->value_divide = CONVERT_FLIP;

				mutex_unlock(&sensor_data_mutex);
#if DEBUG
				if(atomic_read(&data->trace) & MMC_HWM_DEBUG)
				{
					MSE_LOG("Hwm get gesture filp data: %d, %d, %d. divide %d, status %d!\n",
						gest_flip_data->values[0],gest_flip_data->values[1],gest_flip_data->values[2],
						gest_flip_data->value_divide,gest_flip_data->status);
				}
#endif
			}
			break;
...
static int bmm050_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#endif //BMC050_VRV
#ifdef BMC050_GEST_FLIP
		struct hwmsen_object sobj_flip;
#endif/*BMC050_GEST_FLIP*/
	...
	sobj_flip.self = data;
	sobj_flip.polling = 1;
	sobj_flip.sensor_operate = bmm050_flip_operate;
	if(err = hwmsen_attach(ID_GEST_FLIP, &sobj_flip))
	{
		MSE_ERR( "attach fail = %d\n", err);
		goto exit_kfree;
	}
...
#if 1
	unsigned int bmm050_i2c_addr=0;

	if(system_rev<1) // EVB 
	{
		bmm050_i2c_addr = BMM050_I2C_ADDR_EVB;		
	}
	else if(system_rev==1)
	{
		bmm050_i2c_addr = BMM050_I2C_ADDR_REV_A;				
	}
	else
		bmm050_i2c_addr = BMM050_I2C_ADDR_REV_A;		

	bmm050_i2c_info.addr = bmm050_i2c_addr;
#endif 
