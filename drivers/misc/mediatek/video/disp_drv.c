    ...
    while(1) 
    {
        ...
        for (i = 0; i < HW_OVERLAY_COUNT; i++) 
        {
            ...
            else 
            {
                ...
                {
                    if (DISP_IsDecoupleMode() && !atomic_read(&OverlayBypassDirtyFlag)) 
                    {
...
void DISP_DirectOverlayTransfer(void)
{
#if defined(MTK_OVL_DECOUPLE_SUPPORT)
    disp_path_config_dirty dirty_flag;
...
/*                                                                                   */
#if 1  /*                                                       */
void DISP_Crash_UpdateScreen(void)
{
	//lcm_drv->init();
	if (lcm_drv->update)
		lcm_drv->update(0, 0, 240, 320);
    //LCD_CHECK_RET(LCD_SetRoiWindow(0, 0, 240, 320));
	//LCD_CHECK_RET(LCD_FBSetStartCoord(0, 0));
	disp_if_drv->update_screen(FALSE);
	//DISP_UpdateScreen(0,0,240,320);
}
/*                                                                                  */
#endif /*              */
...
DISP_STATUS DISP_StartVsync()
{
    if((LCM_TYPE_DBI == lcm_params->type) ||
      ...
      {
          DISP_DRV_WRAN("DISP_StartSYNC():unknown interface\n");
      }
      return DISP_STATUS_OK;
