...
#if 1 /*                                                                   */
#define LGE_UPDATE_SCREEN_IOCTL     108
#endif  /*                                                                   */
...
#if 1  /*                                                                   */
extern DISP_STATUS DISP_StartVsync();
extern void DISP_display_crash();
...
int mtkfb_display_kcal(int is_white)
{
        ...
        if(down_interruptible(&sem_early_suspend))
        {
            MTKFB_INFO("[FB Driver] can't get semaphore in mtkfb_update_screen()\n");
            return -ERESTARTSYS;
        }
        ...
        if(is_white)
        {
            ...
            memset(mipi_fb_cfg.base, 0xffffffff, mipi_fb_cfg.width * mipi_fb_cfg.height * ((mipi_fb_cfg.bpp) / 8));  // white initialize

            DISP_display_crash();
            //memset((void*)fbVirAddr, 0xFFFFFFFF, MTK_FB_SIZE);
        }
        else
        {
...
EXPORT_SYMBOL(mtkfb_display_kcal);
#endif  /*                                                             */
static void mtkfb_update_screen_impl(void)
{
    BOOL down_sem = FALSE;
...
void mtkfb_crash_late_resume()
{
            ...
            if (down_interruptible(&sem_early_suspend)) {
                printk("[FB Driver] can't get semaphore in mtkfb_crash_late_resume()\n");
...
#if 0
    MTKFB_INFO("[FB Driver] mtkfb_set_overlay_layer():id=%u, en=%u, next_idx=%u, vaddr=0x%x, paddr=0x%x, fmt=%u, d-link=%u, pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u\n",
                        layerInfo->layer_id,
...
#if 1  /*                                                                   */
            DISP_CHECK_RET(DISP_StartVsync());    
#endif  /*                                                                   */
        return 0;
...
#if 1  /*                                                                   */
        case LGE_UPDATE_SCREEN_IOCTL:
        {        
            unsigned char *fb_map;

            struct mtkfb_device *fbdev = (struct mtkfb_device *)mtkfb_fbi->par;

            if(copy_from_user(&fb_map, (void __user*)arg, sizeof(unsigned char*)))
            {
                MTKFB_WRAN("[FB]: copy_from_user failed! line:%d \n", __LINE__);
                return -EFAULT;
            }
...
#if defined(TARGET_S4)
            mipi_fb_cfg.width   = 480;
            mipi_fb_cfg.height  = 800;
            mipi_fb_cfg.stride   = 480;
#else
            ...
            memcpy(mipi_fb_cfg.base, fb_map, mipi_fb_cfg.width * mipi_fb_cfg.height * ((mipi_fb_cfg.bpp) / 8));
...
////////////////////////////////////////////////
    default:
        MTKFB_WRAN("mtkfb_ioctl Not support, info=0x%08x, cmd=0x%08x, arg=0x%08x\n", (unsigned int)info, (unsigned int)cmd, (unsigned int)arg);
        return -EINVAL;
    }
}
...
extern void lcm_driver_detect_on_boot();
#endif  /*                                                                      */
