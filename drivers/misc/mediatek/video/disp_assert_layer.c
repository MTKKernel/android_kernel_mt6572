...
DAL_STATUS DAL_SetLGE_CrashScreen(void)
{
    UINT32 i;
    UINT32 *addr;
    const UINT32 BG_COLOR = MAKE_TWO_RGB565_COLOR(DAL_COLOR_BLUE, DAL_COLOR_BLUE);

    addr = (UINT32 *)dal_fb_addr;

    for(i = 0; i < DAL_GetLayerSize() / sizeof(UINT32); ++ i) {
        *addr ++ = BG_COLOR;
    }

    DAL_SetColor(DAL_COLOR_WHITE, DAL_COLOR_BLUE);
