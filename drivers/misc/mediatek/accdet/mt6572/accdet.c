...
/*                                                                                                */
static int button_press_debounce = 0x1000;
/*                                                                                                */
...
/*                                                                               */
/*                                                                                  */
static int gpio_state = 1; //Plug out
/*                                                                                  */
...
static void inline headset_plug_out(void) 
{
	...
	{
		hook_key_pressed  = 0;
		input_report_key(kpd_accdet_dev, KEY_PLAYPAUSE, hook_key_pressed);
...
/*                                                                                  */
	mt65xx_eint_set_hw_debounce(CUST_EINT_ACCDET_NUM, 1/*CUST_EINT_ACCDET_DEBOUNCE_CN*/);
/*                                                                                  */
#endif
		...
		if(ACC_MEDIA_PLAYPAUSE == accdet_key_event)
            	{
            		if(hook_key_pressed ==1)
            		{
                                ACCDET_DEBUG("[Accdet] KEY_DOWN !\n");
            		}
			else
			{
				ACCDET_DEBUG("[Accdet] KEY_UP !\n");
			}
			/*                                                                                                */
			/*                                                                                                */
			gpio_state = mt_get_gpio_in(GPIO_ACCDET_EINT_PIN); /* 0 = low ( plugged-in ) */
			ACCDET_DEBUG("[Accdet] gpio_state= %d eint_accdet_sync_flag = %d \n",gpio_state,eint_accdet_sync_flag);
			if(gpio_state == 0 && eint_accdet_sync_flag == 1)
			{
			...
			{
				ACCDET_DEBUG("[Accdet] assume this hook was noise\n");
			}
			/*                                                                                                */
            	}
/*                                                                                   */		
...
/*                                                                                  */
			if(1 == eint_accdet_sync_flag) 
			{
				...
				{
					ACCDET_DEBUG("[Accdet] Hook Earjack state is in\n");		
					if(hook_key_pressed == 1)
					{
						hook_key_pressed = 0;
						accdet_status = MIC_BIAS;		
						cable_type = HEADSET_MIC;
						notify_sendKeyEvent(ACC_MEDIA_PLAYPAUSE);
						ACCDET_DEBUG("[Accdet] earjack out in hook press state\n");
						pmic_pwrap_write(ACCDET_DEBOUNCE0, button_press_debounce);
					}
				}
			}
