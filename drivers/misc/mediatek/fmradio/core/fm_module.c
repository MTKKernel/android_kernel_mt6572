...
#if 1 /* sbp_interface */
extern unsigned char g_lge_fm_radio[];
#endif
...
//                                                                                       
extern fm_cust_cfg *mt6627_fm_config_check;
//                          
...
static long fm_ops_ioctl(struct file *filp, fm_u32 cmd, unsigned long arg)
{
    ...
    case FM_IOCTL_POWERUP: {
		...
		//                                                                                       
		WCN_DBG(FM_NTC | MAIN, "MT6627 FM RX RSSI TH_LONG:%d \n", mt6627_fm_config_check->rx_cfg.long_ana_rssi_th);
		//                           
...
static fm_s32 __init mt_fm_init(void)
{
    ...
    #if 1 /* sbp_interface */
    result = strncmp(g_lge_fm_radio, "1", 1);

    if (result == 0)
    {
        printk("jjm_debug : FM Radio Support! Device Register!\n");
    ...
    {
        printk("jjm_debug : FM Radio not Support! Stop device register!\n");
    }
    #else
    //register fm device to platform bus
    ret = platform_device_register(&mt_fm_device);
