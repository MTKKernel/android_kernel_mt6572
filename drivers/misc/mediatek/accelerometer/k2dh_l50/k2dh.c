...
#define K2DH_SHAKING_DETECT_THRESHOLD    (300)     //clubsh cal2 50 -> 200
...
static int k2dh_ResetCalibration(struct i2c_client *client)
{
    ...
    #else  // if SW_CALIBRATION feature disabled, need to fix here
    // TODO : need something
    #endif
...
{  // power mode controled by operating Bandwidth(ODR-Hz)
    ...
    if(enable == TRUE)
    {
        res = k2dh_SetBWRate(client, ODR100);  // operating mode as 100HZ for normal speed
    }
    else
    {
...
static int k2dh_acc_hw_init(struct i2c_client *client, int reset_cali)
{
    ...
    res = k2dh_SetBWRate(client, ODR100);
    if(res != K2DH_SUCCESS )
    {
        return res;
    }
...
static int k2dh_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
    ...
    else
    {
        ...
        sprintf(buf, "%04x %04x %04x", acc[K2DH_AXIS_X], acc[K2DH_AXIS_Y], acc[K2DH_AXIS_Z]);
...
static ssize_t store_trace_value(struct device_driver *ddri, char *buf, size_t count)
{
    ...
    GSE_ERR("data : %d saved_data : %d , buf : %s, count : %d\n", data, atomic_read(&obj->trace), buf, count);
...
#ifdef K2DH_ACCEL_CALIBRATION
static int k2dh_set_offset_target(struct i2c_client *client, unsigned char channel, unsigned char offset)
{
    unsigned char data;
    int comres = 0;
...
static int k2dh_get_cal_ready(struct i2c_client *client, unsigned char *calrdy)
{
    int comres = 0 ;
    unsigned char data;
...
static int k2dh_set_cal_trigger(struct i2c_client *client, unsigned char caltrigger)
{
    int comres = 0;
    unsigned char data;
    struct k2dh_i2c_data *k2dh = i2c_get_clientdata(client);

    if(atomic_read(&k2dh->fast_calib_rslt) != 0)
    {
        atomic_set(&k2dh->fast_calib_rslt, 0);
        GSE_LOG(KERN_INFO "[set] k2dh->fast_calib_rslt:%d\n",atomic_read(&k2dh->fast_calib_rslt));
    }

    return comres;
}

static int k2dh_set_offset_x(struct i2c_client *client, unsigned char    offsetfilt)
{
    int comres = 0;
    unsigned char data;
...
static int k2dh_get_offset_x(struct i2c_client *client, unsigned char *offsetfilt)
{
    int comres = 0;
    unsigned char data;
...
static int k2dh_set_offset_y(struct i2c_client *client, unsigned char    offsetfilt)
{
    int comres = 0;
    unsigned char data;
...
static int k2dh_get_offset_y(struct i2c_client *client, unsigned char *offsetfilt)
{
    int comres = 0;
    unsigned char data;
...
static int k2dh_set_offset_z(struct i2c_client *client, unsigned char    offsetfilt)
{
    int comres = 0;
    unsigned char data;
...
static int k2dh_get_offset_z(struct i2c_client *client, unsigned char *offsetfilt)
{
    int comres = 0;
    unsigned char data;
...
static ssize_t show_cali_value(struct device_driver *ddri, char *buf)
{
    ...
    if(k2dh_get_offset_x(k2dh->client, &offset_x) < 0)
        return -EINVAL;
    if(k2dh_get_offset_y(k2dh->client, &offset_y) < 0)
        return -EINVAL;
    if(k2dh_get_offset_z(k2dh->client, &offset_z) < 0)
        return -EINVAL;
...
static ssize_t store_cali_value(struct device_driver *ddri, char *buf, size_t count)
{
    ...
    else if(3 == sscanf(buf, "%d %d %d", &offset_x, &offset_y, &offset_z))
    {
        ...
        if(k2dh_set_offset_x(k2dh->client, (unsigned char)offset_x) < 0)
            return -EINVAL;
        if(k2dh_set_offset_y(k2dh->client, (unsigned char)offset_y) < 0)
            return -EINVAL;
        if(k2dh_set_offset_z(k2dh->client, (unsigned char)offset_z) < 0)
            return -EINVAL;
    }
    else
    {
...
static ssize_t k2dh_fast_calibration_x_store(struct device_driver *ddri, char *buf, size_t count)
{
    ...
    k2dh_read_accel_xyz(k2dh->client, &acc_cal_pre);

    do{
        mdelay(20);
        k2dh_read_accel_xyz(k2dh->client, &acc_cal);
    ...
    if (k2dh_set_offset_target(k2dh->client, 1, (unsigned char)data) < 0)
        return -EINVAL;

    if (k2dh_set_cal_trigger(k2dh->client, 1) < 0)
        return -EINVAL;

    do{
        mdelay(2);
        k2dh_get_cal_ready(k2dh->client, &tmp);
...
static ssize_t k2dh_fast_calibration_y_store(struct device_driver *ddri, char *buf, size_t count)
{
    ...
    atomic_set(&k2dh->fast_calib_y_rslt, 0);
    k2dh_read_accel_xyz(k2dh->client, &acc_cal_pre);
    ...
    if (k2dh_set_offset_target(k2dh->client, 2, (unsigned char)data) < 0)
        return -EINVAL;

    if (k2dh_set_cal_trigger(k2dh->client, 2) < 0)
        return -EINVAL;
...
static ssize_t k2dh_fast_calibration_z_store(struct device_driver *ddri, char *buf, size_t count)
{
    ...
    atomic_set(&k2dh->fast_calib_z_rslt, 0);
    k2dh_read_accel_xyz(k2dh->client, &acc_cal_pre);
    ...
    if (k2dh_set_offset_target(k2dh->client, 3, (unsigned char)data) < 0)
        return -EINVAL;

    if (k2dh_set_cal_trigger(k2dh->client, 3) < 0)
        return -EINVAL;
...
static long k2dh_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case GSENSOR_IOCTL_SET_CALI:
            ...
            if(k2dh_set_offset_x(obj->client, (unsigned char)sensor_data.x) < 0)
                return -EINVAL;
            if(k2dh_set_offset_y(obj->client, (unsigned char)sensor_data.y) < 0)
                return -EINVAL;
            if(k2dh_set_offset_z(obj->client, (unsigned char)sensor_data.z) < 0)
                return -EINVAL;
            break;
