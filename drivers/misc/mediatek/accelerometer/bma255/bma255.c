...
#if 1  /*                                                                                                 */
#if defined(CUST_EINT_GSE_1_POLARITY)
    #if CUST_EINT_GSE_1_POLARITY == CUST_EINT_POLARITY_LOW
        #define INT1_ACTIVE_LOW
    #endif
#endif
#endif  /*                                                                                                 */
#endif
...
#if 1  /*                                                                                                 */
#define BMA255_INT1_LVL_SEL__POS                0
#define BMA255_INT1_LVL_SEL__LEN                 1
#define BMA255_INT1_LVL_SEL__MSK                0x01
#define BMA255_INT1_LVL_SEL__REG                 BMA255_INT_CONFIG_REG
#endif  /*                                                                                                 */
...
#if 1  /*                                                                                                 */
static int bma255_set_int1_active_mode(struct i2c_client *client, bool high)
{
	int comres = 0;
	unsigned char data;

	comres = bma255_smbus_read_byte(client, BMA255_INT1_LVL_SEL__REG, &data);
	data = BMA255_SET_BITSLICE(data, BMA255_INT1_LVL_SEL, high);
	comres = bma255_smbus_write_byte(client, BMA255_INT1_LVL_SEL__REG, &data);
...
static void bma255_set_int1(struct i2c_client *client)
{
	...
	bma255_set_Int_Enable(client, 9, 1);  // doubletap interrupt enable
	bma255_set_Int_Enable(client, 11, 1);  // flat interrupt enable
	/* maps interrupt to INT1 pin */
	//bma255_set_int1_pad_sel(client, PAD_SINGLE_TAP);
	bma255_set_int1_pad_sel(client, PAD_DOUBLE_TAP);  // doubletap interrupt mapped to int1
	bma255_set_int1_pad_sel(client, PAD_FLAT);  // flat interrupt mapped to int1

#if defined(INT1_ACTIVE_LOW) /*                                                                                                 */
        bma255_set_int1_active_mode(client, false);  // active low
#else
        bma255_set_int1_active_mode(client, true);  // active high
#endif  /*                                                                                                 */
}
...
//			if(sensor_suspend==true)kpd_send_pwrkey();
		...
		case 0x80:
			...
			if (sign_value == 1)
			{
				...
				bma255_smbus_write_byte(client, BMA255_TAP_THRES__REG, &tap_threshold_flat);
			}
			else
			{
				GSE_LOG("non flat position\n");
				bma255_smbus_write_byte(client, BMA255_TAP_THRES__REG, &tap_threshold);
			}
			break;
		default:
...
static void bma255_setup_eint(void)
{
	...
	mt_set_gpio_pull_enable(GPIO_GSE_1_EINT_PIN, GPIO_PULL_ENABLE);
	 ...
	 mt_set_gpio_pull_select(GPIO_GSE_1_EINT_PIN, GPIO_PULL_DOWN);
        ...
        mt65xx_eint_set_hw_debounce(CUST_EINT_GSE_1_NUM, CUST_EINT_GSE_1_DEBOUNCE_CN);
        mt65xx_eint_registration(CUST_EINT_GSE_1_NUM, CUST_EINT_GSE_1_DEBOUNCE_EN, CUST_EINT_GSE_1_POLARITY, bma255_eint_func, 0);
	mt65xx_eint_unmask(CUST_EINT_GSE_1_NUM);
}
#endif
...
static int BMA255_ReadData(struct i2c_client *client, s16 data[BMA255_AXES_NUM])
{
	...
	else
	{
        ...
        #if 1  /*                                                                                            */
            if(!(BMA255_GET_BITSLICE(buf[0], NEW_DATA_X) && BMA255_GET_BITSLICE(buf[2], NEW_DATA_Y)
                   && BMA255_GET_BITSLICE(buf[4], NEW_DATA_Z)))  // when one of x,y,z new_data_flag is 0, read data fail, print the data
            {
                GSE_LOG("Read Data Fail, Reg 0x02 ~ 0x07 : [0x%x], [0x%x], [0x%x], [0x%x], [0x%x], [0x%x]\n", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5]);
            }
        #endif  /*                                                                                           */
...
static int bma255_runCalibration(void)
{
    ...
    if(flatDetection)
    {
        ...
        {
            mdelay(400);
            flatDetection = bma255_fast_calibration_z_store(NULL, "2", 1);
		...
		case SENSOR_DELAY:
			...
			else
			{
				value = *(int *)buff_in;
                            ...
                            if(value >= 70)
                            {
                                sample_delay = BMA255_BW_7_81HZ;  // 64ms;
                            }
                            else if(value >= 40)
				{
                                sample_delay = BMA255_BW_15_63HZ;  // 32ms;
				}
                            else if(value >= 20)
				{
                                sample_delay = BMA255_BW_31_25HZ;  // 16ms;
                            }
                            else if(value >= 10)
                            {
                                sample_delay = BMA255_BW_62_5HZ;  // 8ms;
				}
				else
				{
                                sample_delay = BMA255_BW_125HZ;  // 4ms;
				}
#endif  /*                                                                                              */
