...
void GC0329_Sensor_Init(void)
{
	...
	if(gc0329_module==1)
		{
		GC0329_write_cmos_sensor(0x34, 0x26);		
		}
	else
		{
...
{		
	...
	GC0329_write_cmos_sensor(0xa3 , 0x00);
	GC0329_write_cmos_sensor(0xa8 , 0x20);
	GC0329_write_cmos_sensor(0xa9 , 0x20);
	GC0329_write_cmos_sensor(0xaa , 0x20);
	GC0329_write_cmos_sensor(0xab , 0x00);
	GC0329_write_cmos_sensor(0xac , 0x02);
	GC0329_write_cmos_sensor(0xad , 0x00);
	GC0329_write_cmos_sensor(0xae , 0x20);
	GC0329_write_cmos_sensor(0xaf , 0x20);
	GC0329_write_cmos_sensor(0xb0 , 0x20);
	GC0329_write_cmos_sensor(0xb1 , 0x20);
	GC0329_write_cmos_sensor(0xb2 , 0x20);
	GC0329_write_cmos_sensor(0xb3 , 0x20);
	GC0329_write_cmos_sensor(0xb4 , 0x38);
	GC0329_write_cmos_sensor(0xb5 , 0x20);
	GC0329_write_cmos_sensor(0xb6 , 0x25);
	GC0329_write_cmos_sensor(0xba , 0x4d);
	GC0329_write_cmos_sensor(0xbb , 0x3c);
	GC0329_write_cmos_sensor(0xbc , 0x2d);
	GC0329_write_cmos_sensor(0xc0 , 0x0a);
	GC0329_write_cmos_sensor(0xc1 , 0x07);
	GC0329_write_cmos_sensor(0xc2 , 0x08);
	GC0329_write_cmos_sensor(0xc6 , 0x0a);
	GC0329_write_cmos_sensor(0xc7 , 0x05);
	GC0329_write_cmos_sensor(0xc8 , 0x04);
	GC0329_write_cmos_sensor(0xb7 , 0x20);
	GC0329_write_cmos_sensor(0xb8 , 0x20);
	GC0329_write_cmos_sensor(0xb9 , 0x1a);
	GC0329_write_cmos_sensor(0xbd , 0x07);
	GC0329_write_cmos_sensor(0xbe , 0x00);
	GC0329_write_cmos_sensor(0xbf , 0x28);
        	...
        	if (sensor_id == GC0329_SENSOR_ID)
				{
	            	printk("GC0329 Sensor id = %x\n", sensor_id);
//zhao.li@tcl add
				gc0329_module= mt_get_gpio_in(GPIO26);
#ifdef GC0329YUV_DEBUG
				if(gc0329_module==1)
			{
					//E-welly module
...
BOOL GC0329_set_param_banding(UINT16 para)
{
	switch (para)
	{
		case AE_FLICKER_MODE_50HZ:
			...
			GC0329_write_cmos_sensor(0x2b, 0x03);   //exp level 0  14.28fps
...
BOOL GC0329_set_param_exposure(UINT16 para)
{
	...
	if(gc0329_module==1)
	{
	value_luma = (GC0329_NIGHT_MODE?0x2b:0x02);//0x2b:0x00
	}
	else
	{
	value_luma = (GC0329_NIGHT_MODE?0x2b:0x06);//0x2b:0x00
	}
	value_Y = (GC0329_NIGHT_MODE?0x80:0x68);//0x68:0x50
