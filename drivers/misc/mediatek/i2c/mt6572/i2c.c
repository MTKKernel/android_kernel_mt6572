...
static inline void mt_i2c_dump_info(struct mt_i2c *i2c)
{
	...
	*/
	...
	if(i2c->id == 1) {
		dev_err(i2c->dev,"GPIO%d(SCL):%d,mode%d; GPIO%d(SDA):%d,mode%d\n",
			GPIO101,
			mt_get_gpio_in(GPIO101),
			mt_get_gpio_mode(GPIO101),
			GPIO102,
			mt_get_gpio_in(GPIO102),
			mt_get_gpio_mode(GPIO102));

		dev_err(i2c->dev,"GPIO%d(SCL):%d,mode%d; GPIO%d(SDA):%d,mode%d\n",
			GPIO137,
			mt_get_gpio_in(GPIO137),
			mt_get_gpio_mode(GPIO137),
			GPIO138,
			mt_get_gpio_in(GPIO138),
			mt_get_gpio_mode(GPIO138));
	}
#endif
