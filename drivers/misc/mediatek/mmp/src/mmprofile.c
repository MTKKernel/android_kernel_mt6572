...
#define MMProfileDefaultBufferSize 0x10000
...
static bool mmp_log_on = false;

#define MMP_LOG(prio,fmt, arg...) \
	do { \
		if (mmp_log_on) xlog_printk(prio,TAG_MMPROFILE, "%s(): "fmt"\n", __func__,##arg); \
	}while (0)
...
static MMProfile_Event_t* pMMProfileRingBuffer = NULL;
static unsigned char* pMMProfileMetaBuffer = NULL;
static MMProfile_Global_t MMProfileGlobals __attribute__((aligned(PAGE_SIZE))) = 
{
    .buffer_size_record = MMProfileDefaultBufferSize,
    .new_buffer_size_record = MMProfileDefaultBufferSize,
    .buffer_size_bytes = ((sizeof(MMProfile_Event_t)*MMProfileDefaultBufferSize+(PAGE_SIZE-1))&(~(PAGE_SIZE-1))),
...
{
    unsigned int size;
    MMP_LOG(ANDROID_LOG_DEBUG,,"+enable %u, start %u",MMProfileGlobals.enable,MMProfileGlobals.start);
    MMProfileStart(0);
                                           ...
                                           unsigned int SrcSize, unsigned int DstSize)
{
    ...
    if (SrcLeft < DstLeft)
    {
        memcpy(((unsigned char*)pDst) + *pDstPos, ((unsigned char*)pSrc) + *pSrcPos, SrcLeft);
        *pSrcPos += SrcLeft;
        *pDstPos += SrcLeft;
        return SrcLeft;
    }
    ...
    {
        memcpy(((unsigned char*)pDst) + *pDstPos, ((unsigned char*)pSrc) + *pSrcPos, DstLeft);
        *pSrcPos += DstLeft;
        *pDstPos += DstLeft;
        return DstLeft;
    }
...
void MMProfileGetDumpBuffer(unsigned int Start, unsigned int *pAddr, unsigned int *pSize)
{
    ...
    *pAddr = (unsigned int) MMProfileDumpBlock;
    *pSize = MMProfileDumpBlockSize;
    ...
    if (total_pos < (region_base + sizeof(MMProfile_Global_t)))
    {
		...
		copy_size = MMProfileFillDumpBlock(&MMProfileGlobals, MMProfileDumpBlock, &region_pos, &block_pos, sizeof(MMProfile_Global_t), MMProfileDumpBlockSize);
    ...
    if (total_pos < (region_base + sizeof(MMProfile_EventInfo_t) * (MMProfileGlobals.reg_event_index+1)))
    {
        // Register table
        MMP_Event index;
        ...
        if (Pos + sizeof(MMProfile_EventInfo_t) > region_pos)
        {
            if (region_pos > Pos)
                ...
                SrcPos = 0;
            copy_size = MMProfileFillDumpBlock(&EventInfoDummy, MMProfileDumpBlock, &SrcPos, &block_pos, sizeof(MMProfile_EventInfo_t), MMProfileDumpBlockSize);
            if (block_pos == MMProfileDumpBlockSize)
            {
        ...
        list_for_each_entry(pRegTable, &(MMProfile_RegTable.list), list)
        {
            if (Pos + sizeof(MMProfile_EventInfo_t) > region_pos)
            {
				if (region_pos > Pos)
					...
					SrcPos = 0;
                copy_size = MMProfileFillDumpBlock(&(pRegTable->event_info), MMProfileDumpBlock, &SrcPos, &block_pos, sizeof(MMProfile_EventInfo_t), MMProfileDumpBlockSize);
                if (block_pos == MMProfileDumpBlockSize)
                {
                    mutex_unlock(&MMProfile_RegTableMutex);
                    return;
        ...
        mutex_unlock(&MMProfile_RegTableMutex);
        total_pos = region_base + sizeof(MMProfile_EventInfo_t) * (MMProfileGlobals.reg_event_index+1);
    }
    ...
    if (total_pos < (region_base + MMProfileGlobals.buffer_size_bytes))
    {
        // Primary buffer
        region_pos = total_pos - region_base;
		copy_size = MMProfileFillDumpBlock(pMMProfileRingBuffer, MMProfileDumpBlock, &region_pos, &block_pos, MMProfileGlobals.buffer_size_bytes, MMProfileDumpBlockSize);
		if (block_pos == MMProfileDumpBlockSize)
		{
		    return;
		}
    ...
    MMP_LOG(ANDROID_LOG_DEBUG,"end t=%u,r =%u,block_pos=%u", total_pos,region_base,block_pos);
    *pSize = block_pos;
}
...
static void MMProfileInitBuffer(void)
{
        ...
        (MMProfileGlobals.meta_buffer_size != MMProfileGlobals.new_meta_buffer_size))
    {
        ...
        {
            MMProfileGlobals.buffer_size_record = MMProfileGlobals.new_buffer_size_record;
            MMProfileGlobals.buffer_size_bytes = ((sizeof(MMProfile_Event_t)*MMProfileGlobals.buffer_size_record+(PAGE_SIZE-1))&(~(PAGE_SIZE-1)));
            bResetRingBuffer = 1;
        }
        else if (MMProfileGlobals.buffer_size_record != MMProfileGlobals.new_buffer_size_record)
        {
        ...
        if (bResetRingBuffer)
        {
            pMMProfileRingBuffer = (MMProfile_Event_t*) vmalloc(MMProfileGlobals.buffer_size_bytes);
        }
        MMP_LOG(ANDROID_LOG_DEBUG,"pMMProfileRingBuffer=0x%08x", (unsigned int)pMMProfileRingBuffer);
        ...
        else if (MMProfileGlobals.meta_buffer_size != MMProfileGlobals.new_meta_buffer_size)
        {
        ...
        if (bResetMetaBuffer)
        {
            pMMProfileMetaBuffer = (unsigned char*) vmalloc(MMProfileGlobals.meta_buffer_size);
        }
        MMP_LOG(ANDROID_LOG_DEBUG,"pMMProfileMetaBuffer=0x%08x", (unsigned int)pMMProfileMetaBuffer);
        ...
        if (bResetMetaBuffer)
        {
            ...
            {
                MMProfile_MetaDataBlock_t* pBlock = (MMProfile_MetaDataBlock_t*) pMMProfileMetaBuffer;
...
static MMP_Event MMProfileFindEventInt(MMP_Event parent, const char* name)
{
    ...
    list_for_each_entry(pRegTable, &(MMProfile_RegTable.list), list)
    {
        ...
        {
            if (strncmp(pRegTable->event_info.name, name, MMProfileEventNameMaxLen) == 0)
            {
                return index;
            }
        }
...
static int MMProfileConfigEvent(MMP_Event event, char* name, MMP_Event parent, int sync)
{
    ...
    if ((event >= MMP_MaxStaticEvent) ||
        (event >= MMProfileMaxEventCount) ||
        (event == MMP_InvalidEvent))
    {
        return 0;
    }
    if (sync)
    ...
    pRegTable = (MMProfile_RegTable_t*) vmalloc(sizeof(MMProfile_RegTable_t));
    if (!pRegTable)
    {
        mutex_unlock(&MMProfile_RegTableMutex);
        return 0;
...
static int MMProfileRegisterStaticEvents(int sync)
{
    static unsigned int bStaticEventRegistered = 0;
    ...
    {
        ret = ret && MMProfileConfigEvent(MMProfileStaticEvents[i].event, MMProfileStaticEvents[i].name, MMProfileStaticEvents[i].parent, sync);
    }
...
static void system_time(unsigned int *low, unsigned int *high)
{
    ...
    temp = sched_clock();
	*low = (int32_t)(temp&0xffffffff);
	*high = (int32_t)((temp>>32)&0xffffffff);
	//MMP_LOG(ANDROID_LOG_VERBOSE,"system_time,0x%08x,0x%08x", *high, *low);
}

static void MMProfileLog_Int(MMP_Event event, MMP_LogType type, unsigned int data1, unsigned int data2, unsigned int meta_data_cookie)
{
    if (!MMProfileGlobals.enable)
        return;
    if (bMMProfileInitBuffer && MMProfileGlobals.start && MMProfileGlobals.event_state[event]) 
	{ 
        ...
        if (unlikely(event < 2))
            return;
		index = (atomic_inc_return((atomic_t*)&(MMProfileGlobals.write_pointer))-1)%(MMProfileGlobals.buffer_size_record); 
...
static int MMProfileLogMetaInt(MMP_Event event, MMP_LogType type, MMP_MetaData_t* pMetaData, int bFromUser)
{
    unsigned long retn;
    if (!MMProfileGlobals.enable)
        return 0;
    if (bMMProfileInitBuffer && MMProfileGlobals.start && MMProfileGlobals.event_state[event])
    {
        ...
        if (unlikely(!pMetaData))
            return -1;
        block_size = ((offsetof(MMProfile_MetaDataBlock_t, meta_data) + pMetaData->size)+3)&(~3);
        ...
        {
            MMProfile_MetaDataBlock_t* pNextNode = list_entry(pNode->list.next, MMProfile_MetaDataBlock_t, list);
            if (&(pNextNode->list) == &MMProfile_MetaBufferList)
                pNextNode = list_entry(pNextNode->list.next, MMProfile_MetaDataBlock_t, list);
            list_del(&(pNextNode->list));
            pNode->block_size += pNextNode->block_size;
        }
        // Split the block if left memory is enough for a new block.
        if (((unsigned int)pNode+block_size) < ((unsigned int)pMMProfileMetaBuffer+MMProfileGlobals.meta_buffer_size) &&
            ((unsigned int)pNode+block_size) > ((unsigned int)pMMProfileMetaBuffer+MMProfileGlobals.meta_buffer_size-offsetof(MMProfile_MetaDataBlock_t, meta_data)))
        {
            block_size = (unsigned int)pMMProfileMetaBuffer+MMProfileGlobals.meta_buffer_size-(unsigned int)pNode;
        }
        if ((pNode->block_size - block_size) >= offsetof(MMProfile_MetaDataBlock_t, meta_data))
        {
            MMProfile_MetaDataBlock_t* pNewNode = (MMProfile_MetaDataBlock_t*)((unsigned int)(pNode) + block_size);
            if ((unsigned int)pNewNode >= ((unsigned int)pMMProfileMetaBuffer+MMProfileGlobals.meta_buffer_size))
                pNewNode = (MMProfile_MetaDataBlock_t*)((unsigned int)pNewNode-MMProfileGlobals.meta_buffer_size);
        ...
        MMProfileLog_Int(event, type, pMetaData->data1, pMetaData->data2, MMProfile_MetaDataCookie);
        MMProfile_MetaDataCookie++;
        if (MMProfile_MetaDataCookie == 0)
            MMProfile_MetaDataCookie++;
        if (((unsigned int)(pNode->meta_data) + pMetaData->size) > ((unsigned int)pMMProfileMetaBuffer+MMProfileGlobals.meta_buffer_size))
        {
            unsigned int left_size = (unsigned int)pMMProfileMetaBuffer+MMProfileGlobals.meta_buffer_size-(unsigned int)(pNode->meta_data);
            if (bFromUser)
            {
                retn = copy_from_user(pNode->meta_data, pMetaData->pData, left_size);
                retn = copy_from_user(pMMProfileMetaBuffer, (void*)((unsigned int)(pMetaData->pData)+left_size), pMetaData->size-left_size);
            }
            else
            {
                memcpy(pNode->meta_data, pMetaData->pData, left_size);
                memcpy(pMMProfileMetaBuffer, (void*)((unsigned int)(pMetaData->pData)+left_size), pMetaData->size-left_size);
            }
        }
        else
        {
            if (bFromUser)
                retn = copy_from_user(pNode->meta_data, pMetaData->pData, pMetaData->size);
            else
...
void MMProfileEnableEvent(MMP_Event event, int enable)
{
    ...
    if ((event < 2) || (event >= MMProfileMaxEventCount))
        return;
    MMProfileGlobals.event_state[event] = enable;
}
...
int  MMProfileQueryEnable(MMP_Event event)
{
    ...
    return MMProfileGlobals.event_state[event];
}
...
int MMProfileLogMetaStructure(MMP_Event event, MMP_LogType type, MMP_MetaDataStructure_t* pMetaData)
{
    ...
    if (bMMProfileInitBuffer && MMProfileGlobals.start && MMProfileGlobals.event_state[event])
    {
        ...
        memcpy((void*)((unsigned int)(MetaData.pData)+32), pMetaData->pData, pMetaData->struct_size);
...
int MMProfileLogMetaBitmap(MMP_Event event, MMP_LogType type, MMP_MetaDataBitmap_t* pMetaData)
{
    ...
    if (bMMProfileInitBuffer && MMProfileGlobals.start && MMProfileGlobals.event_state[event])
    {
        ...
        pDst = (char*)((unsigned int)(MetaData.pData)+sizeof(MMP_MetaDataBitmap_t));
        ...
        else
        {
            ...
            MMP_LOG(ANDROID_LOG_DEBUG,"n(%u,%u),o(%u, %u,%d,%u) ", new_width,new_height,
                                   pMetaData->width,pMetaData->height,pMetaData->pitch,pMetaData->bpp);
...
// Debug FS begin
static struct dentry *g_pDebugFSDir = NULL;
static struct dentry *g_pDebugFSStart = NULL;
static struct dentry *g_pDebugFSBuffer = NULL;
static struct dentry *g_pDebugFSGlobal = NULL;
static struct dentry *g_pDebugFSReset = NULL;
static struct dentry *g_pDebugFSEnable = NULL;
static struct dentry *g_pDebugFSMMP = NULL;

static ssize_t mmprofile_dbgfs_reset_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
    MMProfileResetBuffer();
    return 1;
}

static ssize_t mmprofile_dbgfs_start_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
    char str[32];
    int r;
    MMP_LOG(ANDROID_LOG_DEBUG,"start=%d", MMProfileGlobals.start);
...
static ssize_t mmprofile_dbgfs_start_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
    unsigned int str;
    int start;
    ssize_t ret;
    ...
    if ((str&0xFF) == 0x30)
        ...
        start = 1;
    MMP_LOG(ANDROID_LOG_DEBUG,"start=%d", start);
    MMProfileStart(start);
    return ret;
}

static ssize_t mmprofile_dbgfs_enable_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
    char str[32];
    int r;
...
static ssize_t mmprofile_dbgfs_enable_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
    unsigned int str;
    int enable;
    ssize_t ret;
...
static ssize_t mmprofile_dbgfs_buffer_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
    static unsigned int backup_state = 0;
    unsigned int copy_size = 0;
    unsigned int total_copy = 0;
    unsigned int Addr;
    if (!bMMProfileInitBuffer)
        return -EFAULT;
     MMP_LOG(ANDROID_LOG_VERBOSE,"size=%d ppos=%d", size, (int)(*ppos));
    if (*ppos == 0)
    {
...
static ssize_t mmprofile_dbgfs_global_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
    return simple_read_from_buffer(buf, size, ppos, &MMProfileGlobals, MMProfileGlobalsSize);
}

static ssize_t mmprofile_dbgfs_global_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
...
static long mmprofile_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case MMP_IOC_REGEVENT:
        {
            MMProfile_EventInfo_t event_info;
            retn = copy_from_user(&event_info, (void*)arg, sizeof(MMProfile_EventInfo_t));
            event_info.parentId = MMProfileRegisterEvent(event_info.parentId, event_info.name);
            retn = copy_to_user((void*)arg, &event_info, sizeof(MMProfile_EventInfo_t));
        }
        break;
    case MMP_IOC_FINDEVENT:
        {
            ...
            retn = copy_from_user(&event_info, (void*)arg, sizeof(MMProfile_EventInfo_t));
            mutex_lock(&MMProfile_RegTableMutex);
            event_info.parentId = MMProfileFindEventInt(event_info.parentId, event_info.name);
            mutex_unlock(&MMProfile_RegTableMutex);
            retn = copy_to_user((void*)arg, &event_info, sizeof(MMProfile_EventInfo_t));
        }
        break;
    case MMP_IOC_ENABLEEVENT:
        {
            ...
            {
                mutex_lock(&MMProfile_RegTableMutex);
                MMProfileEnableEventRecursive(event, enable);
                mutex_unlock(&MMProfile_RegTableMutex);
            }
            else
                MMProfileEnableEvent(event, enable);
    ...
    case MMP_IOC_DUMPEVENTINFO:
        {
            ...
            MMProfileRegisterStaticEvents(1);
            mutex_lock(&MMProfile_RegTableMutex);
            retn = copy_to_user(pEventInfoUser, &EventInfoDummy, sizeof(MMProfile_EventInfo_t));
            index = MMP_RootEvent;
            list_for_each_entry(pRegTable, &(MMProfile_RegTable.list), list)
            {
                retn = copy_to_user(&pEventInfoUser[index], &(pRegTable->event_info), sizeof(MMProfile_EventInfo_t));
                index++;
            }
            for (; index<MMProfileMaxEventCount; index++)
            {
                retn = copy_to_user(&pEventInfoUser[index], &EventInfoDummy, sizeof(MMProfile_EventInfo_t));
            }
            mutex_unlock(&MMProfile_RegTableMutex);
        }
        break;
    ...
    case MMP_IOC_DUMPMETADATA:
        {
            ...
            list_for_each_entry(pMetaDataBlock, &MMProfile_MetaBufferList, list)
            {
                ...
                {
                    put_user(pMetaDataBlock->cookie, &(pMetaData[meta_data_count].cookie));
                    put_user(pMetaDataBlock->data_size, &(pMetaData[meta_data_count].data_size));
                    put_user(pMetaDataBlock->data_type, &(pMetaData[meta_data_count].data_type));
                    buffer_size += pMetaDataBlock->data_size;
                    meta_data_count++;
                }
            }
            put_user(meta_data_count, (unsigned int*)arg);
            //printk("[mmprofile_ioctl] meta_data_count=%d meta_data_size=%x\n",meta_data_count, buffer_size);
            ...
            list_for_each_entry(pMetaDataBlock, &MMProfile_MetaBufferList, list)
            {
                if (pMetaDataBlock->data_size > 0)
                {
                    ...
                    //printk("[mmprofile_ioctl] MetaRecord: offset=%x size=%x\n", offset-8, pMetaDataBlock->data_size);
                    if (((unsigned int)(pMetaDataBlock->meta_data)+pMetaDataBlock->data_size) > ((unsigned int)pMMProfileMetaBuffer+MMProfileGlobals.meta_buffer_size))
                    {
                        unsigned int left_size = (unsigned int)pMMProfileMetaBuffer+MMProfileGlobals.meta_buffer_size-(unsigned int)(pMetaDataBlock->meta_data);
                        retn = copy_to_user((void*)(arg+offset), pMetaDataBlock->meta_data, left_size);
                        retn = copy_to_user((void*)(arg+offset+left_size), pMMProfileMetaBuffer, pMetaDataBlock->data_size-left_size);
                    }
                    else
                        retn = copy_to_user((void*)(arg+offset), pMetaDataBlock->meta_data, pMetaDataBlock->data_size);
                    offset = (offset+pMetaDataBlock->data_size+3)&(~3);
                    index++;
                }
            }
            put_user(offset-8, (unsigned int*)(arg+4));
            //printk("[mmprofile_ioctl] Finished: offset=%x\n", offset-8);
            mutex_unlock(&MMProfile_MetaBufferMutex);
        }
        break;
    ...
    case MMP_IOC_TRYLOG:
        if ((!MMProfileGlobals.enable) ||
            ...
            (!MMProfileGlobals.event_state[arg]))
            ret = -EINVAL;
        break;
...
static int mmprofile_mmap(struct file *file, struct vm_area_struct * vma)
{
    ...
    if (MMProfileGlobals.selected_buffer == MMProfileGlobalsBuffer)
    {
        ...
        {
            unsigned long pfn;
            //printk("[mmprofile_mmap] mmap pos=0x%08x va=0x%08x pa=0x%08x pfn=0x%08x\n", pos, (unsigned long)(&MMProfileGlobals) + i, __virt_to_phys((unsigned long)(&MMProfileGlobals) + i), __phys_to_pfn(__virt_to_phys((unsigned long)(&MMProfileGlobals) + i)));
            ...
            if (remap_pfn_range(vma, pos, pfn, PAGE_SIZE, vma->vm_page_prot|PAGE_SHARED))
                return -EAGAIN;
            //printk("pfn: 0x%08x\n", pfn);
        }
    }
    else if (MMProfileGlobals.selected_buffer == MMProfilePrimaryBuffer)
    {
        ...
        for (i=0; i<MMProfileGlobals.buffer_size_bytes; i+=PAGE_SIZE, pos+=PAGE_SIZE)
        {
            //printk("[mmprofile_mmap] mmap pos=0x%08x va=0x%08x pfn=0x%08x\n", pos, (void*)((unsigned long)pMMProfileRingBuffer + i), vmalloc_to_pfn((void*)((unsigned long)pMMProfileRingBuffer + i)));
            //flush_dcache_page(vmalloc_to_page((void*)((unsigned long)(pMMProfileRingBuffer) + i)));
            if (remap_pfn_range(vma, pos, vmalloc_to_pfn((void*)((unsigned long)pMMProfileRingBuffer + i)), PAGE_SIZE, vma->vm_page_prot|PAGE_SHARED))
                return -EAGAIN;
        }
    }
    else
...
static int mmprofile_probe(void)
{
    ...
    if (g_pDebugFSDir)
    {
        ...
        g_pDebugFSMMP = debugfs_create_file("mmp", S_IFREG|S_IRUGO, g_pDebugFSDir, NULL, &mmprofile_fops);        
        g_pDebugFSEnable = debugfs_create_file("enable", S_IRUSR|S_IWUSR, g_pDebugFSDir, NULL, &mmprofile_dbgfs_enable_fops);
        g_pDebugFSStart = debugfs_create_file("start", S_IRUSR|S_IWUSR, g_pDebugFSDir, NULL, &mmprofile_dbgfs_start_fops);
        g_pDebugFSBuffer = debugfs_create_file("buffer", S_IRUSR, g_pDebugFSDir, NULL, &mmprofile_dbgfs_buffer_fops);
    ...
    //    if (IS_ERR(filp)) 
    //    {
    //        printk("[mmprofile] NVM: Cannot open configuration file %s\n", CONFIG_MMPROFILE_PATH);
    //        MMProfileEnable(0);
    //    }
    //    else
    //    {
    ...
    //        printk("[mmprofile] NVM: enable=%d start=%d.\n", enable, start);
    //        if (enable == 1)
    //        {
    //            MMProfileEnable(1);
    //            if (start == 1)
