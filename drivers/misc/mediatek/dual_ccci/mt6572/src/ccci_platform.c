...
/* sbp_interface */
#if 0
extern unsigned char g_lge_model_name[];
#endif
...
/* sbp_interface */
#if 0
void lge_convert_ntcode_format_to_qct (FactoryNetworkCode *mtk_atcmd_ntcode, char *qct_testmode_ntcode)
{
	int i = 0;
	int length = 0;
	int ntcode_length = g_lge_number_of_ntcodes;

	FactoryNetworkCode *in;

	char buffer[sizeof(FactoryNetworkCode)];
	char *out = qct_testmode_ntcode;
	
	length += sprintf(out, "\"%d\"", ntcode_length);

	for(i = 0; i < ntcode_length; i++) 
	{
		in = &mtk_atcmd_ntcode[i];
		memset(buffer, 0x00, sizeof(buffer));

		sprintf(buffer, "\"%X%X%X", in->Mcc[0], in->Mcc[1], in->Mcc[2]);
		sprintf(buffer, "%s,%X%X%X", buffer,in->Mnc[0], in->Mnc[1], in->Mnc[2]);
		sprintf(buffer, "%s,%X%X%X%X%X%X%X%X", buffer,in->Gid1[0], in->Gid1[1], in->Gid1[2], in->Gid1[3], in->Gid1[4], in->Gid1[5], in->Gid1[6], in->Gid1[7]);
		sprintf(buffer, "%s,%X%X%X%X%X%X%X%X", buffer,in->Gid2[0], in->Gid2[1], in->Gid2[2], in->Gid2[3], in->Gid2[4], in->Gid2[5], in->Gid2[6], in->Gid2[7]);
		sprintf(buffer, "%s,%X%X\"", buffer, in->Subset[0], in->Subset[1]);

		length += sprintf(out, "%s,%s", out, buffer);

		//printk("jjm_debug : lge_convert_ntcode_format_to_qct : NTCODE[%d]: %s, length: %d\n", i, buffer, length);
	}
}

void lge_convert_ntcode_format_to_int (FactoryNetworkCode *mtk_atcmd_ntcode, int mcc_output, int mnc_output, int subset_output)
{
	FactoryNetworkCode *in;

	int mcc_out = mcc_output;
	int mnc_out = mnc_output;

	in = &mtk_atcmd_ntcode[0];

	mcc_out = in->Mcc[0]*100 + in->Mcc[1]*10 + in->Mcc[2];

	if (in->Mnc[2] == 0x0F)
	{
		mnc_out = in->Mnc[0]*10 + in->Mnc[1];
	}
	else
	{
		mnc_out = in->Mnc[0]*100 + in->Mnc[1]*10 + in->Mnc[2];
	}

	subset_output = in->Subset[0]*10 + in->Subset[1];

	//printk("jjm_debug : lge_convert_ntcode_format_to_int : mcc_out = %d\n", mcc_out);
	//printk("jjm_debug : lge_convert_ntcode_format_to_int : mnc_out = %d\n", mnc_out);
	//printk("jjm_debug : lge_convert_ntcode_format_to_int : subset_output = %d\n", subset_output);
}

char svn_version[10] = "";

void lge_get_svn (void)
{
	int result = 0;
	char *argv[] = {"getprop", svn_version, "ro.lge.swversion_svn", NULL};
	char *envp[] = {"HOME=/", "TERM=linux", "PATH=/sbin:/vendor/bin:/system/sbin:/system/bin:/system/xbin", NULL};

	if ((result = call_usermodehelper("/system/bin/getprop", argv, envp, UMH_WAIT_PROC)) != 0)
	{
		printk("jjm_debug : Fail to get ro.lge.swversion_svn\n");
	}
	else
	{
		printk("jjm_debug : Success to get ro.lge.swversion_svn = %s\n", svn_version);
	}
}
#endif

void lge_convert_ntcode_format_to_string (FactoryNetworkCode *mtk_atcmd_ntcode, char *mcc, char *mnc, char *subset)
{
	int length = 0;
	char buffer[4];
	...
	//printk("jjm_debug : lge_convert_ntcode_format_to_string : MNC: %s, MCC : %s, Subset : %s\n", out1, out2, out3);

	strncpy (g_lge_mcc, out1, 4);
	strncpy (g_lge_mnc, out2, 4);
	strncpy (g_lge_subset, out3, 4);
	//printk("jjm_debug : lge_convert_ntcode_format_to_string : MNC: %s, MCC : %s, Subset : %s\n", g_lge_mcc, g_lge_mnc, g_lge_subset);
}
/* sbp_interface */
...
void config_misc_info(int md_id, unsigned int base[], unsigned int size)
{
	...
	char subset[4] = "";
	bool result = false;
	unsigned short networkCodeListNum = 1;
	FactoryNetworkCode *temp_buf = NULL;
	...
	if(NULL != base) {
		...
		{
			printk("jjm_debug : temp_buf_svn alloc failed!\n");
		}
		else
		{
			...
			if (result == true)
			{
				printk("jjm_debug : temp_buf_svn : Read LGE_FacReadSVN_SBP Success! temp_buf_svn = %s\n", temp_buf_svn);
			}
			else
			{
				printk("jjm_debug : temp_buf_svn : Read Fail! result = %d\n", result);
			}

			memcpy((char*)&(misc_info.feature_4_val[3]), temp_buf_svn, 4); /* SVN */

			printk("jjm_debug : config_misc_info : misc_info.feature_4_val[3] = %s\n", (char *)(&misc_info.feature_4_val[3]));

			kfree(temp_buf_svn);
		}
		
		temp_buf = (FactoryNetworkCode *)kmalloc(EMMC_BLOCK_SIZE, GFP_KERNEL);

		if (temp_buf == NULL)
		{
			printk("jjm_debug : temp_buf alloc failed!\n");
		}
		else
		{
			memset(temp_buf, 0x00, EMMC_BLOCK_SIZE);

			result = LGE_FacReadNetworkCode_SBP (temp_buf, networkCodeListNum, LGE_FAC_NETWORK_CODE_SIZE_SBP);
			...
			{
				printk("jjm_debug : config_misc_info : Read Fail! result = %d\n", result);
			}

			lge_convert_ntcode_format_to_string (temp_buf, mcc, mnc, subset);

			#if 0
			lge_get_svn();
			#endif

			//printk("jjm_debug : config_misc_info : mcc_out = %s, mnc_out = %s, subset_out = %s\n", mcc, mnc, subset);

			kfree(temp_buf);

			memcpy((char*)&(misc_info.feature_4_val[0]), mcc, 4); /* MCC */
			memcpy((char*)&(misc_info.feature_4_val[1]), mnc, 4); /* MNC */
			memcpy((char*)&(misc_info.feature_4_val[2]), subset, 4); /* Subset */

			misc_info.support_mask |= (FEATURE_SUPPORT<<(MISC_NT_CODE*2));

			//printk("jjm_debug : config_misc_info : misc_info.feature_4_val[0] = %s, misc_info.feature_4_val[1] = %s, misc_info.feature_4_val[2] = %s", (char *)(&misc_info.feature_4_val[0]), (char *)(&misc_info.feature_4_val[1]), (char *)(&misc_info.feature_4_val[2]));
		}
		/* sbp_interface */

		//printk("jjm_debug : config_misc_info : misc_info.feature_4_val[0] = %s, misc_info.feature_4_val[1] = %s, misc_info.feature_4_val[2] = %s, misc_info.feature_4_val[3] = %s", (char *)(&misc_info.feature_4_val[0]), (char *)(&misc_info.feature_4_val[1]), (char *)(&misc_info.feature_4_val[2]), (char *)(&misc_info.feature_4_val[3]));
