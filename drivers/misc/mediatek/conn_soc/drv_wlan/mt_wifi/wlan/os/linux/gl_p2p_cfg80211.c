...
#define AP_CAPABILITY_SHORT_PREAMBLE BIT(5)
    ...
    )
{
    ...
    struct ieee80211_mgmt *pMgmt = NULL;
	...
	do {
        ...
        *cookie = prGlueP2pInfo->u8Cookie++;
        ...
        if((((pMgmt->frame_control) & 0x000c) >> 2) == 0 && //MGMT
            (((pMgmt->frame_control) & 0x00f0) >> 4) == 5) //PROBE RESP
        {
            //printk("short preamble set for PROBE RESPONSE\n");
            pMgmt->u.probe_resp.capab_info |= BIT(5);//AP_CAPABILITY_SHORT_PREAMBLE;
        }
