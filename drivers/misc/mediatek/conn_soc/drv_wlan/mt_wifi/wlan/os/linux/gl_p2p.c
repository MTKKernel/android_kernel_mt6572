...
{
    ASSERT(prGlueInfo);
	// normal flow: this func will called first before wlanRemove, and it can do fsmUninit/deactivateNetwork gracefully. 
	//when reset: because tx_thread with fw has stopped, so it can't do these job and the recovery will be dependent on chip system reset.
	//if so, just skip it by flag GLUE_FLAG_HALT(warning: when tx_thread was stop, this flag was not cleared, and NEED TO KEEP IT NOT CLEARED!).
