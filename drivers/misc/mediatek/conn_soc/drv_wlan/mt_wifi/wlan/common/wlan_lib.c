...
#ifdef CONFIG_MTK_TC1_FEATURE//keep alive packet time change to default 30secs//
    {
...
#ifdef CONFIG_MTK_TC1_FEATURE//  special cmd for Only TC1.
    {
        CMD_ACCESS_REG rCmdAccessReg;

        rCmdAccessReg.u4Address = 0x60200A48;
        rCmdAccessReg.u4Data = 0x18A76085;
        wlanSendSetQueryCmd(prAdapter,
            CMD_ID_ACCESS_REG,
            TRUE, //FALSE,
            FALSE, //TRUE,
...
#if CFG_SHOW_MACADDR_SOURCE
                DBGLOG(INIT, INFO, ("Using static MAC address"));
#endif
                    rMacAddr[0] = 0x00;
                    rMacAddr[1] = 0x90;
                    rMacAddr[2] = 0x4c;
    ...
    )
{
    ...
    if(wlanCfgGet(prAdapter, "country", aucValue, "",0) == WLAN_STATUS_SUCCESS) {
        i4Cfg = 1;
    } else if (wlanCfgGet(prAdapter, "COUNTRY", aucValue, "",0) == WLAN_STATUS_SUCCESS) {
        i4Cfg = 1;
    }
    if (i4Cfg == 1) {
