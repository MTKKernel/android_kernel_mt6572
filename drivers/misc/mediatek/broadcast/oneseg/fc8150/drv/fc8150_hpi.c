...
#define FC8150_ADDR_REG     (*(volatile u8 *)(BBM_BASE_ADDR + (BBM_ADDRESS_REG << BBM_BASE_OFFSET)))
#define FC8150_CMD_REG      (*(volatile u8 *)(BBM_BASE_ADDR + (BBM_COMMAND_REG << BBM_BASE_OFFSET)))
#define FC8150_DATA_REG     (*(volatile u8 *)(BBM_BASE_ADDR + (BBM_DATA_REG << BBM_BASE_OFFSET)))
...
{
    //OAL_CREATE_SEMAPHORE(&hBbmMutex, "hpi", 1, OAL_FIFO);
    return BBM_OK;
}
...
int fc8150_hpi_longread(HANDLE hDevice, u16 addr, u32 *data)
{
    ...
    *data = FC8150_DATA_REG;
    *data |= FC8150_DATA_REG << 8;
    *data |= FC8150_DATA_REG << 16;
    *data |= FC8150_DATA_REG << 24;
    //OAL_RELEASE_SEMAPHORE(&hBbmMutex);
...
int fc8150_hpi_wordwrite(HANDLE hDevice, u16 addr, u16 data)
{
    ...
    FC8150_ADDR_REG = (addr & 0xff00) >> 8;
    FC8150_CMD_REG = command;
    FC8150_DATA_REG = (data & 0xff);
    FC8150_DATA_REG = (data & 0xff00) >> 8;
    //OAL_RELEASE_SEMAPHORE(&hBbmMutex);
...
int fc8150_hpi_longwrite(HANDLE hDevice, u16 addr, u32 data)
{
    ...
    FC8150_DATA_REG = (data & 0xff);
    FC8150_DATA_REG = (data & 0xff00) >> 8;
    FC8150_DATA_REG = (data & 0xff0000) >> 16;
    FC8150_DATA_REG = (data & 0xff000000) >> 24;
    //OAL_RELEASE_SEMAPHORE(&hBbmMutex);
