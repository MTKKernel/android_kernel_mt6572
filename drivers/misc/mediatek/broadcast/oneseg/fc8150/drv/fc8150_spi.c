...
static int __devinit fc8150_spi_probe(struct spi_device *spi)
{
    ...
    */
#if defined(CONFIG_MACH_MSM8974_G2_DCM) || defined(CONFIG_MACH_MSM8974_G2_KDDI)   // Not MediaTek platform
    //Parse data using dt.
    if(spi->dev.of_node){
    ...
    dev_err(&spi->dev, "[1seg][irq] SPI sync returned [%d]\n", spi_test_transfer(spi));
#endif //defined(CONFIG_MACH_MSM8974_G2_DCM) || defined(CONFIG_MACH_MSM8974_G2_KDDI)   // Not MediaTek platform
