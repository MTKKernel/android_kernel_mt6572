...
#if ION_DEBUG
struct ion_client *ion_client_create_user(struct ion_device *dev,unsigned int heap_type_mask, const char *name);
...
void *ion_map_kernel(struct ion_client *client, struct ion_handle *handle)
{
		...
		record_param.length = handle->buffer->size;
                ...
                printk(ION_DEBUG_TRACE "[KERNEL ION_MAP_KERNEL]\n");
                record_param.backtrace_num = get_kernel_backtrace((unsigned long *)record_param.backtrace);
		get_kernel_symbol((unsigned long *)record_param.backtrace,record_param.backtrace_num,&(record_param.kernel_symbol[0]));
                record_ion_info((int)1,&record_param);
		printk(ION_DEBUG_TRACE "[KERNEL ION_MAP_KERNEL] DONE\n");
        }
#endif
...
void ion_unmap_kernel(struct ion_client *client, struct ion_handle *handle)
{
	struct ion_buffer *buffer;
		...
		if(current->pid != current->tgid)
                ...
                record_param.address_type = ADDRESS_KERNEL_VIRTUAL;
                printk(ION_DEBUG_TRACE "[KERNEL ION_UMAP_KERNEL]\n");
                record_param.backtrace_num = get_kernel_backtrace((unsigned long *)record_param.backtrace);
                get_kernel_symbol((unsigned long *)record_param.backtrace,record_param.backtrace_num,&(record_param.kernel_symbol[0]));
                record_ion_info((int)1,&record_param);
                printk(ION_DEBUG_TRACE "[KERNEL ION_UMAP_KERNEL] DONE\n");
				     ...
				     const char *name)
{
	...
	#if ION_DEBUG
	{
		...
		printk(ION_DEBUG_TRACE "[KERNEL ION_CLIENT_CREATE]\n");
		record_param.backtrace_num = get_kernel_backtrace((unsigned long *)record_param.backtrace);
		get_kernel_symbol((unsigned long *)record_param.backtrace,record_param.backtrace_num,&(record_param.kernel_symbol[0]));
		record_ion_info((int)1,&record_param);
		printk(ION_DEBUG_TRACE "[KERNEL ION_CLIENT_CREATE] DONE\n");
...
void ion_client_destroy(struct ion_client *client)
{
	...
	idr_destroy(&client->idr);
        ...
        {
                ion_sys_record_t record_param;
		printk(ION_DEBUG_TRACE "[KERNEL ION_CLIENT_DESTROY]\n");
                record_param.client = client;
		record_param.buffer = NULL;
		record_param.handle = NULL;
		record_param.pid = client->pid;
		...
		get_kernel_symbol((unsigned long *)record_param.backtrace,record_param.backtrace_num,&(record_param.kernel_symbol[0]));
                record_ion_info((int)1,&record_param);
		printk(ION_DEBUG_TRACE "[KERNEL ION_CLIENT_DESTROY] DONE\n");
...
int ion_share_dma_buf_fd(struct ion_client *client, struct ion_handle *handle)
{
	...
	if (fd < 0)
		...
		record_param.fd     = fd;
                ...
                printk(ION_DEBUG_TRACE "[KERNEL ION_SHARE]\n");
                record_param.backtrace_num = get_kernel_backtrace((unsigned long *)record_param.backtrace);
		get_kernel_symbol((unsigned long *)record_param.backtrace,record_param.backtrace_num,&(record_param.kernel_symbol[0]));
                record_ion_info((int)1,&record_param);
		printk(ION_DEBUG_TRACE "[KERNEL ION_SHARE] DONE\n");
...
struct ion_handle *ion_import_dma_buf(struct ion_client *client, int fd)
{
	...
	if (ret) {
		...
		if(current->pid != current->tgid)
                ...
                printk(ION_DEBUG_TRACE "[KERNEL ION_IMPORT]\n");
                record_param.backtrace_num = get_kernel_backtrace((unsigned long *)record_param.backtrace);
		get_kernel_symbol((unsigned long *)record_param.backtrace,record_param.backtrace_num,&(record_param.kernel_symbol[0]));
                record_ion_info((int)1,&record_param);
		printk(ION_DEBUG_TRACE "[KERNEL ION_IMPORT] DONE\n");
...
int ion_share_dma_buf_user(struct ion_client *client, struct ion_handle *handle)
{
	...
	struct dma_buf *dmabuf;
	bool valid_handle;
	MMProfileLogEx(ION_MMP_Events[PROFILE_SHARE], MMProfileFlagStart, 1, 1);
	mutex_lock(&client->lock);
	valid_handle = ion_handle_validate(client, handle);
	mutex_unlock(&client->lock);
	if (!valid_handle) {
