...
/*                                                            */
#ifndef CONFIG_LGE_USB_AUTORUN_DRIVER
		...
		struct usb_composite_dev *cdev)
{
	...
	if (!serial_initialized) {
		...
		#ifdef CONFIG_LGE_USB_GADGET_DRIVER
		((struct acm_function_config *)f->config)->instances =  serial_initialized;
		#endif
...
static void acm_function_cleanup(struct android_usb_function *f)
{
	...
	gserial_cleanup();
		...
		#ifdef CONFIG_LGE_USB_GADGET_DRIVER
		((struct acm_function_config *)f->config)->instances = 0;
		#endif
	}
	kfree(f->config);
	f->config = NULL;
					...
					struct usb_configuration *c)
{
    ...
    if (g_qem_check != '1')
	{
		...
		if (ret) {
			printk("[android_usb] %s: gether_setup failed\n", __func__);
			return ret;
		}
	}
#else
...
static DEVICE_ATTR(ethaddr, S_IRUGO | S_IWUSR, ecm_ethaddr_show,
					       ecm_ethaddr_store);
#else
 DEVICE_ATTR(ecm_ethaddr, S_IRUGO | S_IWUSR, ecm_ethaddr_show,
					       ecm_ethaddr_store);
#endif
			    ...
			    const char *buff, size_t size)
{
	...
	int enabled = 0;
	//                                                                           
	struct musb	*musb = gadget_to_musb(cdev->gadget);
...
static ssize_t factory_cable_show(struct device *pdev,
		struct device_attribute *attr, char *buf)
{
	if(_android_dev->fmode >= USB_FACTORY)
	...
	const char *buff, size_t size)
{
	...
	if (enabled)
	{
		dev->fmode = USB_FACTORY_1;
		printk("\n[USB] fmode_store USB_FACTORY_1\n");
	}
	else
	{
		dev->fmode = USB_FACTORY_0;
		printk("\n[USB] fmode_store USB_FACTORY_0\n");
	}
	mutex_unlock(&dev->mutex);
...
{
	LGBmCableId usbCableId = USB_CABLE_ID_NONE;
	extern LGBmCableId g_lgbmBootUsbCableId;
	char usb_driver_desc = 0;

	usbCableId = g_lgbmBootUsbCableId;

	printk("\n[USB]android_set_factory_mode g_lgbmBootUsbCableId %d\n", g_lgbmBootUsbCableId);
	...
	if(g_boot_mode == META_BOOT)
		usb_driver_desc = 0;
	else if (usbCableId == USB_CABLE_ID_56K || usbCableId == USB_CABLE_ID_130K || usbCableId == USB_CABLE_ID_910K)
		usb_driver_desc = 1;
	else if (g_qem_check == '1')
	{
		if (usbCableId == USB_CABLE_ID_OPEN)
			usb_driver_desc = 2;
		else
			usb_driver_desc = 1;
	}
	...
	printk("\n[USB]android_set_factory_mode g_lgbmBootUsbCableId %d, usb_driver_desc %d\n", g_lgbmBootUsbCableId, usb_driver_desc);
	
	if (usb_driver_desc == 1 || usb_driver_desc == 2) {
		_android_dev->fmode = USB_FACTORY; //initialize fmode for loading adb driver
		factory_mode_enabled = true;
		#if 0
		android_factory_desc(0, usb_driver_desc);
		#endif
		android_factory_desc(1, usb_driver_desc);
	}
	else 
	{
		_android_dev->fmode = USB_NORMAL; //initialize fmode for loading adb driver
...
void android_factory_desc(int enable, char usb_desc)
{
	...
	while (b) {
		...
		if (name) {
			err = android_enable_function(dev, name);
			if (err)
				printk("[android_usb] android_usb: Cannot enable '%s'", name);
		}
	}
	...
	if (enable) {
		...
		printk("[android_usb] android_usb: already %s\n",
				dev->enabled ? "enabled" : "disabled");
	}
	return;
}
