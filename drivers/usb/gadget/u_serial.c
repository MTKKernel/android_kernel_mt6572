...
//                                                                    
	...
	while (!list_empty(queue)) {
		...
		if (__ratelimit(&ratelimit)) {
			...
			if(req->actual < 25){
				xlog_printk(ANDROID_LOG_INFO, ACM_LOG, "%s: ttyGS%d: actual=%d, n_read=%d ", __func__, port->port_num, req->actual, port->n_read);
			...
			printk(" --- actual=%d print end android time %d-%02d-%02d %02d:%02d:%02d.%03d\n", i, tm_android.tm_year + 1900, tm_android.tm_mon + 1, tm_android.tm_mday,\
