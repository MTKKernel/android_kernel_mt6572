...
#if defined(TARGET_S4) // ONE_BIN_MEMORY
unsigned long LGE_BSP_MEM_MAX_PHY_ADDR = 0;
resource_size_t get_total_DRAM_size(void);
#endif
...
#if defined (CONFIG_FCI_FC8150_SPI)
static struct spi_board_info spi_board_devs[] __initdata = {
	[0] = {
		.modalias="fci_spi_fc8150",
		.bus_num = 0,
		.chip_select = 0,
		.mode = SPI_MODE_0,
...
#endif /* FCI_FC8150_SPI */ /* CONFIG_LGE_BROADCAST_ONESEG */
#endif
...
/* LGE_BSP_COMMON LGE_CHANGE_S 140120 jongwoo82.lee : LUT KCAL */
#ifdef CONFIG_LGE_LUT_KCAL
struct platform_device lcdkcal_dev = {
    .name = "lge-lcdkcal",
    .id   = -1,
};
#endif 
/* LGE_BSP_COMMON LGE_CHANGE_E 140120 jongwoo82.lee : LUT KCAL */
...
struct platform_device oneseg_dtv_device = {
	.name	       = "isdbt",
	.id            = -1,
};
...
{
/* LGE_CHANGE_S : 2012-10-18 johny.kim@lge.com
 * move kernel commandline to LK boot and skip command checking
 */
#if 1
	char *cs;
...
#endif
/* LGE_CHANGE_E : 2012-10-18 johny.kim@lge.com move kernel commandline to LK boot */
...
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
    ...
    */
    ...
    LGE_BSP_MEM_MAX_PHY_ADDR = (LGE_BSP_MEM_BASE_ADDR + get_total_DRAM_size());
#endif
#endif
...
/* LGE_BSP_COMMON sangcheol.seo@lge.com 131015 : Kernel Crash Handler Porting */
/*=======================================================================*/
/* LGE Crash Handler                                                     */
/*=======================================================================*/
/* LGE_CHANGE_S : lge crash handler
...
#if defined(TARGET_S4) // ONE_BIN_MEMORY
	crash_log_resource[0].start = LGE_BSP_CRASH_LOG_PHY_ADDR;
	crash_log_resource[0].end = (LGE_BSP_CRASH_LOG_PHY_ADDR + LGE_BSP_CRASH_LOG_SIZE - 1);
#endif
...
void __init lge_add_hidden_reset_devices(void)
{
#if defined(TARGET_S4) // ONE_BIN_MEMORY
	lge_hidden_reset_resource[0].start = LGE_BSP_HIDDEN_RESET_PHY_ADDR;
	lge_hidden_reset_resource[0].end = (LGE_BSP_HIDDEN_RESET_PHY_ADDR + LGE_BSP_HIDDEN_RESET_SIZE - 1);
#endif

	printk(KERN_INFO "HIDDEN RESET START ADDR : %d\n", lge_hidden_reset_resource[0].start);
	printk(KERN_INFO "HIDDEN RESET END ADDR   : %d\n", lge_hidden_reset_resource[0].end);

	platform_device_register(&lge_hidden_reset_device);
}
#endif
/* LGE_CHANGE_E : lge crash handler *


/*=======================================================================*/
/* Charge pump driver                                                         */
/*=======================================================================*/
#if defined (CONFIG_LEDS_LM3639)
static struct platform_device mtk_backlight_dev = {
	.name = "charge-pump",
	.id   = -1,};
#endif
#if defined (CONFIG_LGE_MINIABB)
static struct platform_device lge_miniabb_dev = {
	.name = "miniabb",
	.id   = -1,};
static struct platform_device lge_miniabbi2c_dev = {
	.name = "miniabb_i2c",
	.id   = -1,};	
#endif
...
__init int mt_board_init(void)
{
...

	{
		...
		key = 0;
#endif
	/* LGE_CHANGE_S : 20120926 modified unique serial num of kernel to match the serial num of preloadersangchul1.park@lge.com */
		if (key != 0)
			get_serial(key, 6572, serial_number);
		else
	...
	/* LGE_CHANGE_S : 20120926 end sangchul1.park@lge.com */
#if defined(CONFIG_MTK_MTD_NAND)
...
#if defined(CONFIG_LGE_BROADCAST_ONESEG)
        retval = platform_device_register(&oneseg_dtv_device);
        if (retval != 0) {
                return retval;
        }
#endif /* CONFIG_LGE_BROADCAST_ONESEG */
...
/* LGE_BSP_COMMON LGE_CHANGE_S 140120 jongwoo82.lee : LUT KCAL */
#ifdef CONFIG_LGE_LUT_KCAL
	retval = platform_device_register(&lcdkcal_dev);
	if (retval != 0){
		return retval;
	}
#endif 
...
#endif
#if defined (CONFIG_LGE_MINIABB)	
		retval = platform_device_register(&lge_miniabb_dev);
		if (retval != 0) {
			return retval;
		}
		retval = platform_device_register(&lge_miniabbi2c_dev);
		if (retval != 0) {
			return retval;
		}		
#endif
...
{
#if defined(CONFIG_LGE_HANDLE_PANIC) || defined(CONFIG_LGE_HIDDEN_RESET)
    return (bl_mem_sz + RESERVED_MEM_SIZE_FOR_CONNSYS + LGE_BSP_RESERVED_MEM_SIZE);
#else
    return (bl_mem_sz + RESERVED_MEM_SIZE_FOR_CONNSYS);
#endif
}
#endif
