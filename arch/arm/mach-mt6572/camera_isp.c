...
#define LOG_VRB(format, args...)    //xlog_printk(ANDROID_LOG_VERBOSE, K_ISP_TAG, "[%s] " format, __FUNCTION__, ##args)
#define LOG_DBG(format, args...)    //xlog_printk(ANDROID_LOG_DEBUG  , K_ISP_TAG, "[%s] " format, __FUNCTION__, ##args)
#define LOG_INF(format, args...)    //printk(KERN_DEBUG K_ISP_TAG format, ##args) //[LGE_UPDATE] [yonghwan.lym@lge.com] [2014-03-01] Reduce the Shutter Lag
#define LOG_WRN(format, args...)    //xlog_printk(ANDROID_LOG_WARN   , K_ISP_TAG, "[%s] WARNING: " format, __FUNCTION__, ##args)
#define LOG_ERR(format, args...)    xlog_printk(ANDROID_LOG_ERROR  , K_ISP_TAG, "[%s, line%04d] ERROR: " format, __FUNCTION__, __LINE__, ##args)
#define LOG_AST(format, args...)    //xlog_printk(ANDROID_LOG_ASSERT , K_ISP_TAG, "[%s, line%04d] ASSERT: " format, __FUNCTION__, __LINE__, ##args)
...
unsigned int backup_fe = 0; //[LGE_UPDATE][yonghwan.lym@lge.com][2014-04-30] MCLK set to low when MCLK is disabled.
...
void ISP_MCLK1_EN(MBOOL En)
{
    ...
    if(En)
    {
        ISP_WR32(SENINF_BASE + 0x304,backup_fe); //[LGE_UPDATE][yonghwan.lym@lge.com][2014-04-30] MCLK set to low when MCLK is disabled.
        temp |= 0x20000000;
        ISP_WR32(SENINF_BASE + 0x300,temp);
    }
    else
    {
        backup_fe = ISP_RD32(SENINF_BASE + 0x304); //[LGE_UPDATE][yonghwan.lym@lge.com][2014-04-30] MCLK set to low when MCLK is disabled.
        ISP_WR32(SENINF_BASE + 0x304,(backup_fe & 0xffffffc0));	//[LGE_UPDATE][yonghwan.lym@lge.com][2014-04-30] MCLK set to low when MCLK is disabled.
        temp &= 0xDFFFFFFF;
        ISP_WR32(SENINF_BASE + 0x300,temp);
    }
